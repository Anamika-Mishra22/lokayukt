...
namespace App\Http\Controllers\api\APS;
...
class APSComplaintsController extends Controller
{
...
//         'complaints.approved_rejected_by_aps',
...
//                 ->where('rep.forward_to_aps', $psId);
//         })

//         ->orWhere(function ($inner) {
//             $inner->where('complaints.approved_rejected_by_rk', 1)
//                 ->whereNull('complaints.approved_rejected_by_aps');
//         });
...
//     //  ->where('complaints.approved_rejected_by_lokayukt', 1)
//     //       ->whereNotNull('rep.forward_to_aps')
//     //       ->where('rep.forward_to_aps', $user);
// $query->where('complaints.approved_rejected_by_rk', 1)
//     ->where('complaints.approved_rejected_by_lokayukt', 1)
//     // ->whereNotNull('rep.forward_to_aps')
//     ->where(function ($q) use ($user, $parentId) {
//         $q->where('rep.forward_to_aps', $user)
   ...
   $query->where('rep.status', 'Forwarded')
          ->where(function ($q) use ($user, $parentId) {
              ...
              $q->where('rep.forward_to_aps', $user)
                ...
                // APS ke under CIO
                ->orWhere('rep.forward_to_cio_io', $parentId);
     ...
     $query->where('rep.status', 'Forwarded')
          ->where(function ($q) use ($user, $parentId) {
              ...
              $q->where('rep.forward_to_aps', $user)
                ...
                // APS ke under CIO
                ->orWhere('rep.forward_to_io', $parentId);
...
$records = $query->orderBy('complaints.updated_at','DESC')
->get();
// ->toSql();
...
->when($roleParent === 'supervisor' && $userParentSubrole->sub_role_id == 15, function ($query) use ($parentId){
...
->when($roleParent === 'supervisor' && $userParentSubrole->sub_role_id == 19, function ($query) use ($parentId){
...
->when($roleParent === 'supervisor' && $userParentSubrole->sub_role_id == 20, function ($query) use ($parentId){
             ...
             ->join('complaint_actions as rep','complaints.id', '=', 'rep.complaint_id')
                ...
                ->join('complaint_actions as rep','complaints.id', '=', 'rep.complaint_id')
                    ...
                    ->where('fee_exempted', 0)
                     ->distinct('complaints.id')
                    ->count();
     }elseif($roleParent ==="supervisor" && $userParentSubrole->sub_role_id == 24){
         $todayCount = DB::table('complaints')
             ->join('complaint_actions as rep','complaints.id', '=', 'rep.complaint_id')
                    ->where('in_draft', 0)
                    ->whereDate('rep.created_at', today())
                     ->distinct('complaints.id')
                     ->where('rep.forward_to_so', $parentId)
                    ->count();
...
public function getSubROleUsers()
{
    ...
    public function forwardComplaintbyAPS(Request $request,$complainId){
        //    dd($request->all());
        // $user = Auth::user()->id;
        // dd($usersubrole);
         $parentId = null;
        ...
        if(isset($complainId) && $request->isMethod('post') && $request->filled('otp')){
            ...
            // dd($cmp);
               ...
               if($cmp){
                $cmp->approved_rejected_by_aps = 1;  
                ...
                // $cmp->remark = $remark;
                    ...
                    if($cmp->save()){
                         ...
                         $apcAction = new ComplaintAction();
                            $apcAction->complaint_id = $complainId;
                            $apcAction->forward_by_aps = $userId;
                            ...
                            if($roleFwd === "lok-ayukt" || $roleFwd === "up-lok-ayukt" || $roleFwd === "ps"){
                        ...
                        ->where('forward_by_aps', $userId)
                        ->latest('id')
                        ->update([
                            'approved_rejected_by_aps' => 1
                        ]);
...
// }
    ...
    //              if ($roleParent === 'lok-ayukt') {
    //                 $q->where('cm.approved_rejected_by_aps','1')
    //                   ->where('rep.forward_by_aps', $userId);
    ...
    //                     // ->toSql();
    //                     // ->where('approved_rejected_by_aps', 1 )
    //                     ->distinct('cm.id')
    //                   ->get();
...
//                 ->where('rep.forward_by_aps', $userId)
//                 ->where('rep.approved_rejected_by_aps', 1);
//         });
...
public function allComplainsapproved()
{
    ...
    $userParentSubrole = Auth::user()->userParentRole ?? null;
        ...
        ->whereExists(function ($sub) use ($userId) {
            ...
            $sub->select(DB::raw(1))
                ...
                ->whereRaw('rep.id = (
                    ...
                    WHERE ca.complaint_id = cm.id
                )')

                // APS ne latest action me approve + forward kiya ho
                ->where('rep.forward_by_aps', $userId)
                ->where('rep.approved_rejected_by_aps', 1);
        });
    ...
    public function assignToAPs(Request $request,$complainId){
        //    dd($request->all());
        $user = Auth::user()->id;
