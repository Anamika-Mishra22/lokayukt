...
public function getLokayuktPSUsers()
{
    $usersByRole = User::with('role')
        ->whereHas('role') // 🔥 ensures role exists
        ->where('parent_user_id',233)
        ->get()
        ->groupBy(function ($user) {
            return $user->role->name;
        });

    if (!empty($usersByRole['ps'])) {
        return response()->json($usersByRole['ps']);
    } else {
        return response()->json(["message" => "Data Not Found"]);
    }
}
 ...
 public function forwardComplaintbyLokayukt(Request $request,$complainId){
    ...
    public function rejectComplaintByLokayukt(Request $request,$complainId){
        ...
        if(isset($complainId) && $request->isMethod('post')){
            ...
            if($cmp){
                $cmp->status = 'Rejected';
                $cmp->rejected_by = $user;
                    if($cmp->save()){
                    $apcAction = new ComplaintAction();
                    $apcAction->complaint_id = $complainId;
                     $apcAction->forward_by_lokayukt = $user;
                    $apcAction->status = 'Rejected';
                    $apcAction->remarks = "This complaint Reject by Lokayukt";
                    $apcAction->save();
                }
            }
    ...
    public function approvedFeeByLokayukt(Request $request, $complaint_id){
        ...
        if(isset($id) && $request->isMethod('post')){
                ...
                if($apc->save()){
                    ...
                    $apcAction->forward_to_uplokayukt = $request->forward_to;
                    $apcAction->save();
                    $apc->deal_by = "uplokayukt";
                    $apc->deal_person_id = $request->forward_to;
                    $apc->save();
                }
    ...
    public function assignToPs(Request $request, $id)
{
    $userId = Auth::user()->id;
    ...
    try {
        ...
        // ✅ Check user PS hai ya nahi + parent_user_id match
        $psUser = User::where('id', $request->assign_to)
            ->where('parent_user_id', $userId) // ya 233 agar fix hai
        ...
        if (!$psUser) {
            return response()->json([
                'status' => false,
                'message' => 'Invalid PS User'
            ], 400);
        }

        // ✅ Update complaint
        $complaint->assigned_to_ps = $request->assign_to;
        $complaint->save();
