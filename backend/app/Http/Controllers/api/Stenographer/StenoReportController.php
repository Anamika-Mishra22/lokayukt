...
class StenoReportController extends Controller
{
...
public function search(Request $request)
{
    $query = DB::table('complaints as c')
        ->leftJoin('district_master_new as dd', 'c.district_id', '=', 'dd.district_code')

        // 👤 Complainant (main)
        ->leftJoin('complainants as cmlan', function ($join) {
            $join->on('c.id', '=', 'cmlan.complaint_id')
                 ->where('cmlan.is_main', 1);
        })

        // 👤 Respondent (main)
        ->leftJoin('respondents as resp', function ($join) {
            $join->on('c.id', '=', 'resp.complaint_id')
        ...
        ->select(
            'c.*',
            'dd.district_name as complaint_district',
            'cmlan.complainant_name as complainant_name',
            'resp.respondent_name as respondent_name',
            'dd1.district_name as complainant_district'
        );

    // 🔍 Complaint ID
    if ($request->complaint_id) {
        $query->where('c.id', $request->complaint_id);
    }

    // 🔍 District
    if ($request->district) {
        $query->where('c.district_id', $request->district);
    }

    // 🔍 Complainant Name
    if ($request->complainant_name) {
        $query->where('cmlan.name', 'like', '%' . $request->complainant_name . '%');
    }

    // 🔍 Respondent Name
    if ($request->respondent_name) {
        $query->where('resp.name', 'like', '%' . $request->respondent_name . '%');
    }
    ...
    if ($request->from && $request->to) {
        $query->whereBetween('c.created_at', [$request->from, $request->to]);
    }
    ...
    if ($request->search) {
        $query->where(function ($q) use ($request) {
            $q->where('c.id', 'like', '%' . $request->search . '%')
              ->orWhere('cmlan.name', 'like', '%' . $request->search . '%')
              ->orWhere('resp.name', 'like', '%' . $request->search . '%');
        });
    }
    ...
    public function request_complaints(Request $request)
{
    ...
    try {
        ...
        $validator = Validator::make($request->all(), [
            ...
            // 'status'       => 'required|in:request,allow,received',
            'reason'       => 'required',
        ...
        // ✅ Insert into request_complaints
        $complaint = DB::table('request_complaints')->insertGetId([
            'user_id'      => $user_id,
        ...
        DB::table('request_complaints_remarks')->insert([
            'request_complaint_id' => $complaint,
            'user_id'              => $user_id,
            'remark'               => $request->remark,
            'created_at'           => now(),
            'updated_at'           => now()
        ...
        return response()->json([
            'status'  => true,
            'message' => 'Request complaint created with remark'
        ]);
