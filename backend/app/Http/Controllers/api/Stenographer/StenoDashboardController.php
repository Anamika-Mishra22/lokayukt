...
class StenoDashboardController extends Controller
{
        ...
        //             ->orderByDesc('cmp.id');
                        ...
                        ->where(function ($q) use ($psId) {
                            ...
                            ->orWhere(function ($inner) {
                                $inner->where('cmp.approved_rejected_by_rk', 1)
                                    ->whereNull('cmp.approved_rejected_by_st');
                            });
                        })
          ...
          $query1 =   (clone $baseQuery)
            ...
            $query2=$query2->whereYear('cmp.created_at', $date->year)
                        ->where('cmp.approved_rejected_by_rk', 1)
                        ->where('cmp.approved_rejected_by_lokayukt', 0)
                        ->where('cmp.approved_rejected_by_st', 1)
                        ->whereMonth('cmp.created_at', $date->month)
            ...
            $query4 = $query4->where('cmp.status','Rejected')
                  ...
                  $query = (clone $baseQuery)
                    ...
                    ->whereMonth('cmp.created_at', $date->month)
                    ->where('rep.forward_to_aps', $userId);
                    // ->count();
                ...
                //     ->where('cmp.approved_rejected_by_rk', 1)
                //     ->where('cmp.approved_rejected_by_lokayukt', 1)
                //     ->where('cmp.approved_rejected_by_st', 0)
                //     ->where(function($q) use ($userId) {
                //         $q->whereNull('rep.forward_by_aps')
                //         ->orWhere('rep.forward_by_aps', '!=', $userId);
                //     })
                //     ->distinct('cmp.id')
                //     ->orderByDesc('cmp.id');
              $query1  = (clone $baseQuery)
...
//         $q->whereNull('cmp.approved_rejected_by_st')
//           ->orWhere('cmp.approved_rejected_by_st', '!=', 1);
//     };
...
//     /*
//     |--------------------------------------------------------------------------
//     | DISPOSED COMPLAINS (approved_rejected_by_st = 1)
//     |--------------------------------------------------------------------------
//     */
