...
use App\Models\User;
use App\Models\LetterPersonalDispatch;
use App\Models\LetterRtiDispatch;
...
class DispatchComplaintsController extends Controller
{
    ...
    $query = DB::table('complaints')
     ...
     $query->where('rep.status', 'Forwarded')
                  ->where('complaints.approved_rejected_by_dispatch', 0)
...
// }
 ...
 public function forwardComplaintbyLokayukt(Request $request,$complainId){
     ...
     public function forwardComplaintbyDispatch(Request $request,$complainId){
        //    dd($request->all());
        // $user = Auth::user()->id;
        // dd($usersubrole);
         $parentId = null;
        ...
        if(isset($complainId) && $request->isMethod('post')){
            ...
            // dd($cmp);
               ...
               if($cmp){
                $cmp->approved_rejected_by_dispatch = 1;  
                ...
                // $cmp->remark = $remark;
                    ...
                    if($cmp->save()){
                         ...
                         $apcAction = new ComplaintAction();
                            ...
                            }elseif($roleFwd === "operator"){
                                     $apcAction->forward_to_rk = $request->forward_to;
                            }
    ...
    //    $userSubrole = Auth::user()->subrole->name; 
     ...
     $complainDetails->where('form_status', 1)
            // ->where('approved_rejected_by_ro', 1)
            //             ->where('approved_rejected_by_d_a',1)
             ->where('cm.approved_rejected_by_dispatch', 1);
                        // ->orWhere('approved_rejected_by_lokayukt',1);
            //              ->where(function($q){
            //                 $q->where('approved_rejected_by_so_us',1)
            //                 ->Orwhere('approved_rejected_by_ds_js', 1);               
                        //  });
    ...
    public function addDispachLeters(Request $request)
    {
      ...
      $letter =  Letter::create([
          ...
          $compDoc = new ComplainDocuments();
                ...
                $compDoc->added_by = $userId;
                $compDoc->type = $request->letter_type;
                $compDoc->title = 'Dispatch Letter';
    ...
    public function addDispachPersonalLeters(Request $request)
    {
        $request->validate([
        ...
        // Save DB
      $letter =  LetterPersonalDispatch::create([
    ...
    public function addDispachRtilLeters(Request $request)
    {
        $request->validate([
            'file_id' => 'required|integer',
            'letter_type'  => 'required|string|max:50',
        ...
        // Save DB
      $letter =  LetterRtiDispatch::create([
            'file_id' => $request->file_id,
            'letter_type'  => $request->letter_type,
            'medium'      => $request->medium,
            'subject'      => $request->subject,
    ...
    return response()->json([
        ...
        public function allDispatchPersonalLetters(Request $request){
          $user = Auth::user()->id;
          $records = DB::table('dispach_personal_letters')
                    // ->join('complaint_actions as rep', 'complaints.id', '=', 'rep.complaint_id')
                    // ->where('rep.forward_to_dispatch', $user)
                    ->get();
    ...
    return response()->json([
        ...
        public function allDispatchRtiLetters(Request $request){
          $user = Auth::user()->id;
          $records = DB::table('dispach_rit_letters')
                    // ->join('complaint_actions as rep', 'complaints.id', '=', 'rep.complaint_id')
                    // ->where('rep.forward_to_dispatch', $user)
                    ->get();
    ...
    return response()->json([
        ...
        public function allDispatchRtiLettersId(Request $request){
          $user = Auth::user()->id;
          $records = DB::table('dispach_rit_letters')
                    // ->join('complaint_actions as rep', 'complaints.id', '=', 'rep.complaint_id')
                    // ->where('rep.forward_to_dispatch', $user)
