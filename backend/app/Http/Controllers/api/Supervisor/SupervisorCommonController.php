    ...
    public function getPdfPages(Request $request)
{
    ...
    // 🔸 CASE 2: Folder path
    if ($request->filled('path')) {   // ✅ FIXED

        $path = $request->path;

        if (!is_dir($path)) {   // ✅ better check
            return response()->json([
                'status' => false,
                'message' => 'Invalid path'
            ], 400);
        ...
        try {
            // ✅ FIX: recursive iterator (instead of File::allFiles)
            ...
            foreach ($iterator as $file) {
                ...
                if ($file->isFile() && strtolower($file->getExtension()) === 'pdf') {
                    ...
                    try {
                        ...
                        $data[] = [
                            'file_name' => $file->getFilename(),
                            'file_path' => $file->getPathname(), // ✅ debug ke liye useful
                            'total_pages' => $pages
                        ];
    ...
    // 🔸 FINAL fallback (missing tha tumhare code me)
    return response()->json([
        'status' => false,
        'message' => 'Please provide either file or path'
    ], 400);
}
