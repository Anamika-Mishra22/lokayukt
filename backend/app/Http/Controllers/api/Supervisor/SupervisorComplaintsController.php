...
use App\Models\PersonalFileDocument;
        ...
        if ($userSubrole) {
            ...
            switch ($userSubrole) {
                ...
                case "sec":
                    ...
                    //   ->whereOr('forward_to_uplokayukt', 1);
                    // $query->groupBy('rep.target_date');    
                    $query->where('approved_rejected_by_pro', 0)
                        ->where('rep.status', 'Forwarded')
                        ->whereNotNull('rep.forward_to_pro')
                        ->where('rep.forward_to_pro', $user);
                    //  $query->where('rep.type', 2)
                    //                 ->where('rep.status', 'Report Requested')
                    //                 ->whereNotNull('rep.forward_to_pro')
                    //                  ->where('rep.forward_to_pro',$user);
                    ...
                    $todayCount = DB::table('complaints')
                        ...
                        ->where('in_draft', 0)
                        ->whereDate('repN.created_at', today())
                        ->where('repN.forward_to_pro', $user)
                        ->distinct('complaints.id')
                        ->count();
                    ...
                    $older7DaysCount = DB::table('complaints')
                        ...
                        ->where('repN.created_at', '<', now()->subDays(7))
                        ->where('complaints.approved_rejected_by_rk', 1)
                        ->where('complaints.approved_rejected_by_pro', 0)
                        ->where('repN.forward_to_pro', $user)
                        ->distinct('complaints.id')
                        ->count();
                ...
                case "so":
                    ...
                    //   ->whereOr('forward_to_uplokayukt', 1);
                    // $query->groupBy('rep.target_date');    
                    $query->where('approved_rejected_by_so', 0)
                        ->where('rep.status', 'Forwarded')
                        ->whereNotNull('rep.forward_to_so')
                        ->where('rep.forward_to_so', $user);
                    //  $query->where('rep.type', 2)
                    //                 ->where('rep.status', 'Report Requested')
                    //                 ->whereNotNull('rep.forward_to_so')
                    //                  ->where('rep.forward_to_so',$user);
                    ...
                    $older7DaysCount = DB::table('complaints')
                        ...
                        ->where('repN.created_at', '<', now()->subDays(7))
                        ->where('complaints.approved_rejected_by_rk', 1)
                        ->where('complaints.approved_rejected_by_so', 0)
                        ->where('repN.forward_to_so', $user)
                        ->distinct('complaints.id')
                        ->count();
...
if (!empty($psApsIds)) {
    ...
    $query->whereNotExists(function ($sub) use ($psApsIds) {
        ...
        $sub->select(DB::raw(1))
            ...
            ->whereColumn('ca.complaint_id', 'complaints.id')
    ...
    public function getUserWithPs()
    {
        ...
        $users = collect()
            ...
            ->merge($usersByRole['supervisor'] ?? collect())
            ->merge($usersByRole['uplokayukt'] ?? collect())
            ->merge($usersByRole['aps'] ?? collect());
                ...
                if ($cmp) {
                    ...
                    if ($cmp->save()) {
                        ...
                        if (in_array($roleFwd, ['lok-ayukt', 'up-lok-ayukt', 'ps'])) {
                            ...
                            if ($roleFwd === 'lok-ayukt') {
                                $apcAction->forward_to_lokayukt = $request->forward_to;
                                if($cmp->approved_rejected_by_lokayukt == 1){
                                    $cmp->approved_rejected_by_lokayukt = 0;
                                    $cmp->save();
                                }
                            } else if ($roleFwd === 'up-lok-ayukt') {
                                $apcAction->forward_to_uplokayukt = $request->forward_to;
                                if($cmp->approved_rejected_by_lokayukt == 0){
                                ...
                                $cmp->assign_to_ps = $request->forward_to;
                                // PS ke liye pending state
                                $cmp->approved_rejected_by_ps = 0;
                                $cmp->save();
                            }
                    ...
                    if ($cmp->save()) {
                        ...
                        if (in_array($roleFwd, ['lok-ayukt', 'up-lok-ayukt', 'ps'])) {
                            ...
                            switch ($subroleFwd) {
                                case 'ds':
                                    $apcAction->forward_to_ds = $request->forward_to;
                                    if($cmp->approved_rejected_by_ds == 1){
                                        $cmp->approved_rejected_by_ds = 0;
                                        $cmp->save();
                                    }
                                    break;
                                case 'js':
                                    $apcAction->forward_to_js = $request->forward_to;
                                    if($cmp->approved_rejected_by_js == 1){
                                        $cmp->approved_rejected_by_js = 0;
                                        $cmp->save();
                                    }
                                    break;
                                case 'us':
                                    $apcAction->forward_to_us = $request->forward_to;
                                    if($cmp->approved_rejected_by_us == 1){
                                        $cmp->approved_rejected_by_us = 0;
                                        $cmp->save();
                                    }
                                    break;
                                case 'sec':
                                    $apcAction->forward_to_sec = $request->forward_to;
                                    if($cmp->approved_rejected_by_sec == 1){
                                        $cmp->approved_rejected_by_sec = 0;
                                        $cmp->save();
                                    }
                                    break;
                                ...
                                case 'cio-io':
                                    $apcAction->forward_to_cio_io = $request->forward_to;
                                    if($cmp->approved_rejected_by_cio_io == 1){
                                        $cmp->approved_rejected_by_cio_io = 0;
                                        $cmp->save();
                                    }
                                    break;
                                ...
                                case 'so':
                                    $apcAction->forward_to_so = $request->forward_to;
                                    if($cmp->approved_rejected_by_so == 1){
                                        $cmp->approved_rejected_by_so = 0;
                                        $cmp->save();
                                    }
                                    break;
            ...
            if ($request->otp === $userOtp) {
                ...
                if ($cmp) {
                    ...
                    if ($cmp->save()) {
                        ...
                        if (in_array($roleFwd, ['lok-ayukt', 'up-lok-ayukt', 'ps','aps'])) {
                            ...
                            if ($roleFwd === 'lok-ayukt') {
                               ...
                               $apcAction->forward_to_aps = $request->forward_to;
                                ...
                                $cmp->assign_to_aps = $request->forward_to;
                                // PS ke liye pending state
                                // $cmp->approved_rejected_by_aps = 0;
                                $cmp->save();
                            } else {
        ...
        switch ($userSubrole) {
            ...
            case "pro":
                $complainDetails->where('rep.status', 'Forwarded')
                    // ->whereNotNull('rep.forward_to_pro')
                    // ->where('approved_rejected_by_rk', 1)
                    ->where('approved_rejected_by_pro', 1)
                    ->where('cm.approved_rejected_by_pro', 1)
                    ->where('rep.forward_by_pro', $user);
                //    ->where('forward_to_lokayukt', 1)
                //   ->whereOr('forward_to_uplokayukt', 1);
                break;
            case "ro-aro":
    ...
    public function getPersonalNotes(Request $request, $id)
    {
        if (!$request->isMethod('get')) {
            return response()->json([
                'status' => false,
        ...
        // $file = EmployeeUploadFiles::where()->get();
        ...
        //         // 3. Forwarded users (NO permission required)
        ...
        //     return response()->json([
        //         'status' => false,
        //         'message' => 'File not found or you do not have permission to view its notes.',
        //         'data' => null
        //     ], 404);
        // }
        ...
        // } elseif (
        //     $file->is_forward == 1 &&
        //     ($file->permission_user_id == $userId || $file->user_id == $userId)
        // ) {
        //     // Forwarded user (NO permission required)
        //     $canView = 1;
        //     $canEdit = 0; // optional

        // } elseif ($file->permissions->first()) {
        //     // Permission वाले users
        ...
        // Fetch notes **only added by the user or forwarded to the user**
        $notes = EmployeeComplainNotes::where('employee_file_id', $id)
            // ->where(function ($q) use ($userId) {
            //     $q->where('added_by', $userId)
            //         ->orWhere('forward_to', $userId);
            // })
            ->leftJoin('users', 'employee_complain_notes.forward_by', '=', 'users.id')
            ->select(
                'employee_complain_notes.*',
                'users.name as forwarded_by_name',
                'users.email as forwarded_by_email'
            )
            ->orderBy('employee_complain_notes.id', 'desc')
            ->get();

        // Attach permission info
        // $notes->each(function ($note) use ($canView, $canEdit) {
        //     $note->can_view = $canView;
        //     $note->can_edit = $canEdit;
        // });
    ...
    public function uploadDocumentPersonalFile(Request $request)
    {
        // dd($request->complain_id);
        // $user = $request->user()->id;
        $added_by = Auth::user()->id;
        ...
        $validation = Validator::make($request->all(), [
            ...
            'file_id.required' => 'Complaint Id is required.',
            'type.required' => 'Complaint description is required.',
            'title.required' => 'Letter Subject is Required',
            'file.required' => 'File is Required',
        ]);
        ...
        if (isset($request->file_id)) {
            $compDoc = new PersonalFileDocument();
            $compDoc->file_id = $request->file_id;
            $compDoc->added_by = $added_by;
            $compDoc->type = $request->type;
            $compDoc->title = $request->title;
            $file = 'doc_' . uniqid() . '.' . $request->file('file')->getClientOriginalExtension();
