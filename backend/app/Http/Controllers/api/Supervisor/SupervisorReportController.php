...
if ($userSubrole) {
    ...
    switch ($userSubrole) {
        case "so-us":
            // $records->where('ca.forward_to_so', $user_id);
            break;
...
public function search(Request $request)
{
    ...
    if ($request->complainant_name) {
        $query->where('cmlan.complainant_name', 'like', '%' . $request->complainant_name . '%');
    }

    // 🔍 Respondent Name
    if ($request->respondent_name) {
        $query->where('resp.respondent_name', 'like', '%' . $request->respondent_name . '%');
    }
...
public function request_complaints(Request $request)
{
    ...
    try {
        ...
        return response()->json([
            'status'  => true,
            'message' => 'Request complaint created with remark & log added'
        ]);
