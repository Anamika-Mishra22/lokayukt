...
class UpLokAyuktComplaintsController extends Controller
{
    ...
    //                 //  ->whereNull('rep.forward_by_uplokayukt')
    ...
    ->where('approved_rejected_by_rk', 1)
    ->where('complaints.approved_rejected_by_lokayukt', 1)
    ->where('complaints.approved_rejected_by_uplokayukt', 0)
    ->where('rep.forward_to_uplokayukt', $user);
    // ->where(function($q){
    //     $q->where('rep.forward_by_uplokayukt','<')
    //       ->orWhereNull('rep.complaint_id'); // <-- no record case
    // });
...
public function getUpLokayuktUsers(){
   ...
   public function getSubROleUsers(){
    ...
    //    $userSubrole = Auth::user()->subrole->name; 
     ...
     $complainDetails->where('form_status', 1)
      ->distinct('complaints.id')
                  ->where('rep.forward_by_uplokayukt', $user)
                  ->where('approved_rejected_by_rk', 1)
                  ->where('approved_rejected_by_lokayukt', 1)
                  ->where('approved_rejected_by_uplokayukt', 1);
            // ->where('approved_rejected_by_rk', 1)
            //             ->where('approved_rejected_by_lokayukt',1);
                        // ->where('approved_rejected_by_lokayukt',1)
                        //  ->where(function($q){
      ...
      public function pullBackByupLokayukt(Request $request,$complainId){
        ...
        if(isset($complainId) && $request->isMethod('post')){
            ...
            if($cmp){
                ...
                $cmp->approved_rejected_by_lokayukt = 0;
                $cmp->deal_by = 'lokayukt';
                $cmp->deal_person_id = 233;
     ...
     public function addNotes(Request $request)
    {
     ...
     public function rejectComplaintByUpLokayukt(Request $request,$complainId){
        //    dd($request->all());
        $user = Auth::user()->id;
