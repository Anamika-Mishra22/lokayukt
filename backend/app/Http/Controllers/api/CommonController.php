...
public function fetchLeavesDetails()
{
    ...
    return ApiResponse::generateResponse('success','Leaves approval list',$cat);
}
     ...
     public function fetchLeavesPersonalDetails(){
        ...
        $cat = EmployeeUploadFiles::with('user:id,name')
                ->with('userName')
                ->where('type','Personal File')->where('approved_rejected_by_sec',1)->orderBy('id','desc')->get();
        // dd($cat->toArray());
        return ApiResponse::generateResponse('success','Leaves fetch successfully',$cat);
    }
    ...
    //     $cat = EmployeeUploadFiles::with('user:id,name')
    //             ->with('userName')
    //             ->where('type','RTI')->where('approved_rejected_by_pio',1)->orderBy('id','desc')->get();
    //     // dd($cat->toArray());
    //     return ApiResponse::generateResponse('success','Leaves fetch successfully',$cat);
    // }
    ...
    {
        $userId = auth()->id();
        $is_pio = auth()->user()->is_pio;
        $cat = '';
        if($is_pio === 1 ){
        $cat = EmployeeUploadFiles::with('user:id,name')
            ->with('userName')
            ->whereIn('type', ['RTI','APPEAL'])
            ->where('approved_rejected_by_pio', 1)
        ...
        //  dd($userId,$is_pio,$cat);
        return ApiResponse::generateResponse(
            'success',
            'Leaves fetch successfully',
            $cat
        );
    }
    public function fetchUserRtiDetails()
    {
        $userId = auth()->id();
        $is_pio = auth()->user()->is_pio;
        
        
            $cat = EmployeeUploadFiles::whereIn('type', ['RTI','APPEAL'])
            ->where('approved_rejected_by_pio', 1)
            ->where('permission_user_id', $userId)
            ->orderBy('id', 'desc')
            ->get();
    ...
    public function fetchUserRtiDetailsIds()
    {
        $userId = auth()->id();
        $is_pio = auth()->user()->is_pio;
    ...
    public function fetchLeavesRtiDetailsById($id)
{
    $cat = EmployeeUploadFiles::with('user:id,name','actions')
    // ->where('type', 'RTI','APPEAL')
    ->where('id', $id)
    ->first();
  ...
  public function accessFilePermission(Request $request)
{
  ...
  public function getRolesSupervisors(){
    ...
    $user=User::whereIn('role_id',['3','7'])->select('id','name','role_id','sub_role_id')
                ->with('role','subrole')
                ->get();
    // dd($user->toArray());
