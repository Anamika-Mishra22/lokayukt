...
const Dashboard = ({ userRole = "aps" }) => {
  ...
  const fetchWeeklyData = async () => {
    try {
      ...
      const response = await api.get('/aps/getWeeklyGraph');
  ...
  const fetchDashboardData = async (monthParam) => {
    try {
      ...
      const dashResponse = await api.get(`/aps/dashboard/${monthParam}`);
      ...
      const monthlyResponse = await api.get('/aps/montly-complaint');
      ...
      const statusResponse = await api.get('/aps/status-distribution');
      ...
      const deptResponse = await api.get('/aps/department-wise-complaint');
      ...
      // 5. District-wise API
      const districtResponse = await api.get('/aps/district-wise-company-type');
...
const handleDateChange = (date) => {
  ...
  const datePickerCustomStyles = `
    ...
    .custom-datepicker-wrapper .react-datepicker__month-wrapper {
      display: grid;
      grid-tapslate-columns: repeat(3, 1fr);
      gap: 0.25rem;
    }
 ...
 <div className="grid grid-cols-1 relative top-2 sm:grid-cols-6 md:grid-cols-2 lg:grid-cols-4 xl:grid-cols-6 gap-4">
        ...
        <div 
          onClick={()=>{
          // navigate("/aps/approved-complaints")
        }}
        ...
        <div
        //   onClick={()=>{
        //   navigate("/aps/pending-complaints")
        // }}
