...
const UserID = Cookies.get("UserID"); // <-- Added for Edit Condition
...
setTimeout(() => {
  ...
  useEffect(() => {
    const fetchDocs = async () => {
      try {
        const res = await api.get(`/aps/get-document/${complaint?.id}`);
  ...
  const fetchNotes = async () => {
    ...
    try {
      setLoading(true);
      const res = await api.get(`/aps/get-notes/${complaint.id}`);
      if (res.data.status) {
  ...
  const fetchPdfPath = async (filename) => {
    try {
      const res = await api.get(`/aps/get-file-preview/${complaint.id}`);
  ...
  const handleEditClick = async (noteId) => {
    try {
      setLoading(true);
      const res = await api.get(`/aps/note/${noteId}`);
  ...
  "pdf-content-div"
    ...
    try {
      let res;
      if (editNoteId) {
        res = await api.put(`/aps/notes/${editNoteId}`, payload);
      } else {
        res = await api.post("/aps/add-notes", payload);
      }
                    ...
                    <div className="flex items-center justify-between w-full mt-1 flex-wrap">
                      ...
                      <div className="flex items-center gap-1">
                        ...
                        <button 
                          ...
                          className="text-blue-600 hover:text-blue-800 text-xs font-medium border border-blue-200 px-2 py-0.5 rounded bg-blue-50"
                        >
                          Edit
                        </button>
                      ) : null}
...
`}</style>
  ...
  <div className="flex pt-4 mt-auto pdf-bottom-signature w-full">
    <div className="text-start text-xs text-gray-600">
      <p>{name} (APS) {new Date().toLocaleDateString("en-IN")}</p>
    </div>
  </div>
</div>
