...
//     const resNew = await api.get("/aps/all-approved-complaints");
...
//  console.log(res.roleParent)
  ...
  const getAllComplaints = async () => {
    const res = await api.get("/aps/all-approved-complaints");
...
const handleSendToUPLokayukt = async () => {
  ...
  try {
    ...
    await api.post(
      `/aps/forward-to-uplokayukt/${complaintToApprove.id}`,
      {
  ...
  const getComplaintTypes = async () => {
    const res = await api.get("/aps/complainstype");
    return res.data.data;
  };
  ...
  const handleConfirmApproval = async () => {
    ...
    try {
      const response = await api.post(
        `/aps/approved-by-ro/${complaintToApprove.id}`
      );
