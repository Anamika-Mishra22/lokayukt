...
const EditDraft = () => {
    ...
    const fetchData = async () => {
      ...
      try {
        ...
        const [
          ...
          api.get(`/operator/edit-draft/${id}`),
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const response = await api.post(`/operator/update-draft/${id}`, formDataToSend, {
    ...
    <div className="p-3 sm:p-4 md:p-6 bg-gray-50 min-h-screen">
      ...
      <div className="mb-4 sm:mb-6">
        <div className="flex flex-col space-y-3 sm:flex-row sm:items-center sm:justify-between sm:space-y-0">
          <div>
            <h1 className="text-xl sm:text-2xl font-bold text-gray-900">Edit Draft Complaint</h1>
            <p className="text-xs sm:text-sm text-gray-600">ड्राफ्ट संपादन फॉर्म</p>
          </div>
          <div className="flex flex-col space-y-2 sm:flex-row sm:space-y-0 sm:space-x-3">
            <button 
              onClick={() => navigate(`/operator/draft/view/${id}`)}
