...
const releaseComplaintMutation = useMutation({
  ...
  const markAsReceivedMutation = useMutation({
    mutationFn: async ({ complaintId, remarkData }) => {
      const res = await api.get(`/lokayukt/return-complain-by-lokayukt/${id}`, {
        params: {
...
const forwardComplaintMutation = useMutation({
    // 👇 YAHAN DHYAN DEIN: bracket ke andar 'otpData' add kiya hai
    mutationFn: async ({ complaintId, forwardTo, remarkData, otpData }) => { 
      const res = await api.post(
        `/lokayukt/forward-by-lokayukt/${complaintId}`,
        {
          ...
          sent_through_rk: sent_through_rk ? 1 : 0,
          otp: otpData, // 👇 Ab ye otpData yahan sahi se aayega
        },
      );
      return res.data;
    },
    onSuccess: (data) => {
      ...
      setOtp("");
      setConfirmConfig({ open: false, type: null });
      setFieldErrors({}); // Success hone par errors clear kar do
    },
    onError: (error) => {
      ...
      if (errorData?.errors) {
        ...
        toast.error(errorData?.message || "Failed to forward");
      }
    },
  });

  // Change PS API & Mutation
  const { data: psListData, isLoading: isPsListLoading } = useQuery({
    queryKey: ["ps-list"],
    queryFn: async () => {
      const res = await api.get("/lokayukt/get-ps");
      return res.data?.data || res.data || [];
    },
    enabled: showChangePsModal,
  });

  const changePsMutation = useMutation({
    mutationFn: async ({ complaintId, psId }) => {
      const res = await api.post(`/lokayukt/assign-to-ps/${complaintId}`, {
        assign_to: psId,
      });
      return res.data;
    },
    onSuccess: (data) => {
      toast.success("PS Changed Successfully");
      queryClient.invalidateQueries({ queryKey: ["complaint-details", id] });
      setShowChangePsModal(false);
      setSelectedNewPs("");
    },
    onError: (error) => {
      toast.error(error?.response?.data?.message || "Failed to change PS");
    },
  });

  const handleChangePsSubmit = () => {
    if (!selectedNewPs) {
      toast.error("Please select a new PS");
      return;
    }
    changePsMutation.mutate({ complaintId: id, psId: selectedNewPs });
  };
    ...
    : "उपलब्ध नहीं"}
            ...
            <div className="border-t p-4">
              <div className="flex flex-col sm:flex-row gap-3 justify-between">
                ...
                <button
                    onClick={() => setShowChangePsModal(true)}
                    className="px-4 py-2 border border-indigo-300 text-indigo-700 bg-indigo-50 rounded hover:bg-indigo-100 text-sm font-medium ml-2"
                  >
                    Change PS
                  </button>
                 ...
                 
{
 ...
 <div className="mt-8 flex flex-col gap-3 bg-blue-50/50 border border-blue-100 p-4 rounded-xl">
  ...
  <div className="flex flex-col gap-3 text-sm text-slate-700 mt-1 pl-1">
    ...
    <p>
      <span className="font-semibold text-slate-900">Pull Back</span>
      <span className="mx-2 text-slate-400">—</span>
       Complaint send back to Record Keeper.
    </p>

    <p>
      <span className="font-semibold text-slate-900">Reject</span>
      <span className="mx-2 text-slate-400">—</span>
      Reject button from reject complaint.
    </p>

    <p>
      <span className="font-semibold text-slate-900">Release</span>
      <span className="mx-2 text-slate-400">—</span>
       Release the complaint from the assigned officer (PS/RO/ARO).
    </p>

    <p>
      <span className="font-semibold text-slate-900">Dispose</span>
      <span className="mx-2 text-slate-400">—</span>
      Dispose button for close/Final Decision.
    </p>

    <p>
      <span className="font-semibold text-slate-900">Return with Remarks</span>
      <span className="mx-2 text-slate-400">—</span>
      Send the complaint back with remarks.
    </p>

    <p>
      <span className="font-semibold text-slate-900">Send / Mark</span>
      <span className="mx-2 text-slate-400">—</span>
       Forward the complaint to another officer for further processing.
    </p>
  ...
  <div className="mb-5 mt-3">
    ...
    <input
      ...
      onChange={(e) => {
        ...
        setFieldErrors({ ...fieldErrors, assigned_date: null });
      }}
  ...
  <div className="mb-5 mt-3">
    ...
    <input
      ...
      onChange={(e) => {
        ...
        setFieldErrors({ ...fieldErrors, target_date: null });
      }}
              ...
              <div className="space-y-4">
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                  ...
                  Father Name
                    ...
                    <p className="text-gray-800 font-medium">
                      {complaintData.complainants?.[0]?.father_name || "उपलब्ध नहीं"}
                    </p>
                  </div>
                  ...
                  <p className="text-xs text-gray-500 uppercase font-semibold">
                    ...
                    <p className="text-gray-800 font-medium">
                      {complaintData.complainants?.[0]?.occupation || "उपलब्ध नहीं"}
                      </p>
                      </div>
                </div>
                </div>
                      ...
                      <tbody className="bg-white divide-y divide-gray-200">
                        {complaintData.complainants.map((comp, idx) => (
                          ...
                          >
                            ...
                            <td className="px-6 py-4 kruti-input whitespace-nowrap text-sm text-gray-700 border-r border-gray-200">
                              {comp.district_name || "miyC/k ugha"}
                            </td>
                            <td className="px-6 py-4 kruti-input  whitespace-nowrap text-sm text-gray-700 border-r border-gray-200">
                              {comp.occupation || "-"}
                            </td>
...
<td className="px-6 py-4">
  ...
  <div className="fixed inset-0 bg-black/60 flex items-center justify-center z-50">
    ...
    <div className="bg-white w-[90%] h-[90%] rounded-xl shadow-xl flex flex-col">
                    ...
                    >
                      ...
                      <div className="grid grid-cols-1 sm:grid-cols-2 gap-x-4 gap-y-4">
                        <div>
                          ...
                          {resp?.respondent_name || "उपलब्ध नहीं"}
                          </p>
                        </div>
                        <div>
                          <p className="text-xs text-gray-500 font-semibold">
                          ...
                          <p className="text-sm text-gray-800">
                            {resp?.current_address || "उपलब्ध नहीं"}
                            </p>
                        </div>
                      </div>
                      </div>
                        ...
                        {complaintData.respondant.map((resp, idx) => (
                          ...
                          >
                            ...
                            <td className="px-4 kruti-input py-3 text-sm text-gray-600 border-r border-gray-100 whitespace-nowrap">
                              {resp.district_name || "miyC/k ugha"}
                            </td>
                            <td className="px-4 py-3 kruti-input text-sm text-gray-600 border-r border-gray-100 whitespace-nowrap">
      ...
      {showChangePsModal && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black bg-opacity-40">
          <div className="bg-white rounded-lg shadow-lg w-full max-w-sm p-5 relative animate-slideDown">
            <div className="flex justify-between items-center mb-4">
              <h3 className="text-lg font-semibold text-gray-800">Change PS</h3>
              <button
                onClick={() => setShowChangePsModal(false)}
            ...
            <div className="mb-5">
              <label className="block text-sm font-medium text-gray-700 mb-2">
                Select New PS <span className="text-red-500">*</span>
              </label>
              
              {isPsListLoading ? (
                <div className="w-full px-3 py-2 border border-gray-300 rounded text-gray-500 text-sm bg-gray-50">
                  Loading PS List...
                </div>
              ) : (
                <select
                  value={selectedNewPs}
                  onChange={(e) => setSelectedNewPs(e.target.value)}
                  className="w-full px-3 py-2 border border-gray-300 rounded focus:outline-none focus:ring-2 focus:ring-blue-500 text-sm"
                >
                  <option value="">-- Select PS --</option>
                  {psListData?.map((ps) => (
                    <option key={ps.id} value={ps.id}>
                      {ps.name || ps.user_name || `PS ${ps.id}`}
                    </option>
                  ))}
                </select>
              )}
            ...
            <div className="flex justify-end gap-3">
              ...
              <button
                onClick={handleChangePsSubmit}
                disabled={changePsMutation.isPending || !selectedNewPs}
              ...
              >
                {changePsMutation.isPending ? "Changing..." : "Change"}
              </button>
            </div>
          </div>
        </div>
