...
//         `/lokayukt/approved-by-ro/${complaintToApprove.id}`
//       );
...
//                           <span className="ml-1">
//                             {complaint.cause_date || "उपलब्ध नहीं"}
//                           </span>
...
//                           {/* NAYA CODE: Assigned to PS */}
//   {complaint.assign_to_ps && (
//     <span className="px-2 py-0.5 bg-teal-100 text-teal-800 rounded text-[11px] font-medium whitespace-nowrap">
//       {complaint.assign_to_ps == 268 ? "Shri Pradeep Kumar Srivastava" :
//        complaint.assign_to_ps == 271 ? "Shri Ashish Kumar Rawat" :
//        complaint.assign_to_ps == 235 ? "PS (235)" : // Note: 235 ka naam aapne nahi bataya tha, yahan update kar lein
//        `PS ID: ${complaint.assign_to_ps}`}
//     </span>
//   )}
...
const AllComplaints = () => {
  ...
  const [selectedCaseType, setSelectedCaseType] = useState("");
  const [selectedNature, setSelectedNature] = useState("");
  // 👇 Yahan PS ka naya state add kiya gaya hai 👇
  ...
  useEffect(() => {
    ...
    if (selectedFeeStatus !== "") {
      filtered = filtered.filter(
        (complaint) => complaint.fee_exempted?.toString() === selectedFeeStatus
      );
    }
    ...
    // 👇 Naya PS Filter Logic 👇
    if (selectedPS !== "") {
      filtered = filtered.filter(
        (complaint) => complaint.assign_to_ps?.toString() === selectedPS
      );
    }
      ...
      <div className="w-full h-screen flex bg-gray-50 rounded-md overflow-hidden">
        <div className="w-full bg-white flex flex-col overflow-hidden">
          <div className="px-3 sm:px-4 py-3 border-b flex-shrink-0 bg-white">
            ...
            <div className="flex flex-col lg:flex-row lg:items-center lg:justify-between gap-2 text-xs">
              {/* 👇 flex-wrap add kiya gaya hai taaki filters hide na hon 👇 */}
              <div className="grid grid-cols-2 sm:grid-cols-3 lg:flex lg:flex-row lg:flex-wrap gap-2">
                <select
                  className="border border-gray-300 px-2 py-1 rounded-md focus:outline-none focus:ring-1 focus:ring-blue-500 text-xs"
                  value={selectedStatus}
                  onChange={(e) => setSelectedStatus(e.target.value)}
                  ...
                  {districtData?.map((item) => {
                    ...
                    return (
                      <option
                        key={item.id}
                        value={districtName}
                        className="kruti-input text-[16px]"
                      >
                        {districtName}
                      </option>
                    );
...
{/* 
                ...
                >
                   <option value="">Nature: All</option>
                  <option value="complaint">परिवाद</option>
...
<option value="today">Today's Case</option>
                ...
                {/* 👇 NAYA DROPDOWN YAHAN ADD KIYA HAI 👇 */}
                <select
                  value={selectedPS}
                  onChange={(e) => setSelectedPS(e.target.value)}
                  className="border border-gray-300 px-2 py-1 rounded-md text-xs"
                >
                  <option value="">Assigned to PS</option>
                  <option value="268">Shri Pradeep Kumar Srivastava</option>
                  <option value="271">Shri Ashish Kumar Rawat</option>
                  {/* <option value="235">PS (235)</option> */}
                </select>
              </div>
                    ...
                    <div className="flex flex-col sm:flex-row items-start justify-between gap-3 sm:gap-4">
                      ...
                      <div className="flex flex-col items-start sm:items-end gap-2 flex-shrink-0 w-full sm:w-auto">
                        <div className="flex gap-1.5">
                          ...
                          {/* {complaint.assign_to_ps && (
                            <span className="px-2 py-0.5 bg-teal-100 text-teal-800 rounded text-[11px] font-medium whitespace-nowrap">
                              {complaint.assign_to_ps == 268
                                ? "Shri Pradeep Kumar Srivastava"
                                : complaint.assign_to_ps == 271
                                ? "Shri Ashish Kumar Rawat"
                                : complaint.assign_to_ps == 235
                                ? "PS (235)"
                                : `PS ID: ${complaint.assign_to_ps}`}
                            </span>
                          )} */}
