...
const Notes = ({ complaint }) => {
  ...
  useEffect(() => {
    const fetchDocs = async () => {
      try {
        const res = await api.get(`/admin/get-document/${complaint?.id}`);
  ...
  const fetchNotes = async () => {
    ...
    try {
      setLoading(true);
      const res = await api.get(`/admin/get-notes/${complaint.id}`);
      if (res.data.status) {
  ...
  const fetchPdfPath = async (filename) => {
    try {
      const res = await api.get(`/admin/get-file-preview/${complaint.id}`);
  ...
  const handleFinalSubmit = async () => {
    ...
    try {
      const res = await api.post("/admin/add-notes-employee", payload);
