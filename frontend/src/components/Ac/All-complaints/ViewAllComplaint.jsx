...
const ViewAllComplaint = () => {
  ...
  const {
    ...
    queryFn: async () => {
      const res = await api.get(`/ac/view-complaint/${id}`);
  ...
  const {
    ...
    queryFn: async () => {
      try {
        // 1️ Send To My Pool → API: /ac/get-users
        if (forwardType === "self") {
          const res = await api.get("/ac/get-users");
          return res.data?.data || res.data || [];
        }

        // 2️ Send To Other Pool → API: /ac/get-lokayukt-uplokayukt
        const res = await api.get("/ac/get-lokayukt-uplokayukt");
...
return flatList.filter(
  ...
  const dispose = useMutation({
    mutationFn: async ({ complaintId, remarkData }) => {
      // ध्यान दें: यहाँ मैंने API endpoint /ac/dispose-complain कर दिया है
      const res = await api.post(`/ac/dispose-complain/${id}`, {
  ...
  mutationFn: async ({ complaintId }) => {
    const res = await api.post(
      `/ac/reject-complaint-by-ac/${complaintId}`
    );
    return res.data;
  },
  onSuccess: () => {
  ...
  const pullBackMutation = useMutation({
    mutationFn: async ({ complaintId }) => {
      const res = await api.post(`/ac/pull-back-by-ac/${complaintId}`);
      return res.data;
    },
    onSuccess: (data) => {
  ...
  const assignToSelfMutation = useMutation({
    mutationFn: async ({ complaintId }) => {
      const res = await api.post(`/ac/assign-by-ac/${complaintId}`);
      return res.data;
    },
    onSuccess: (data) => {
  ...
  const returnWithRemarksMutation = useMutation({
    mutationFn: async ({ complaintId, remarkData }) => {
      const res = await api.post(`/ac/return-complain-by-ac/${complaintId}`, {
        remark: remarkData,
      });
      return res.data;
    },
 ...
 const forwardComplaintMutation = useMutation({
    mutationFn: async ({ complaintId, forwardTo, remarkData, otpData }) => {
      const res = await api.post(`/ac/forward-complain-by-ac/${complaintId}`, {
                ...
                <div>
                  <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                    ...
                    <div>
                      ...
                      <p className="text-gray-800 text-sm">
                        {capitalizeFirstLetter(complaintData.category) || "उपलब्ध नहीं"}
                      </p>
                    </div>
                  </div>
                </div>
