      ...
      const canvas = await html2canvas(activeRef.current, {
        ...
        onclone: (document) => {
          ...
          if (el) {
            ...
            styles.innerHTML = `
            .pdf-page-container { min-height: 910px !important; padding-bottom: 20px !important; padding-top: 10px !important; }
...
setTimeout(() => {
  ...
  useEffect(() => {
    const fetchDocs = async () => {
      try {
        const res = await api.get(`/ac/get-document/${complaint?.id}`);
  ...
  const fetchNotes = async () => {
    ...
    try {
      setLoading(true);
      const res = await api.get(`/ac/get-notes/${complaint.id}`);
      if (res.data.status) {
  ...
  const fetchPdfPath = async (filename) => {
    try {
      const res = await api.get(`/ac/get-file-preview/${complaint.id}`);
  ...
  const handleEditClick = async (noteId) => {
    try {
      setLoading(true);
      const res = await api.get(`/ac/note/${noteId}`);
  ...
  "pdf-content-div"
    ...
    try {
      let res;
      if (editNoteId) {
        res = await api.put(`/ac/notes/${editNoteId}`, payload);
      } else {
        res = await api.post("/ac/add-notes", payload);
      }
...
`}</style>
  ...
  <div className="flex pt-4 mt-auto pdf-bottom-signature w-full">
    <div className="text-start text-xs text-gray-600">
      <p>{name} (AC) {new Date().toLocaleDateString("en-IN")}</p>
    </div>
  </div>
</div>
