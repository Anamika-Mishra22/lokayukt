...
//  const roleParent = Cookies.get("roleParent");
//  const roleParent = async () => {
//     const resNew = await api.get("/ac/all-approved-complaints");
...
//  console.log(res.roleParent)
  ...
  const getAllComplaints = async () => {
    const res = await api.get("/ac/all-approved-complaints");
...
const handleSendToUPLokayukt = async () => {
  ...
  try {
    ...
    await api.post(
      `/ac/forward-to-uplokayukt/${complaintToApprove.id}`,
      {
  ...
  const getDistrict = async () => {
    const res = await api.get("/ac/all-district");
  ...
  const getComplaintTypes = async () => {
    const res = await api.get("/ac/complainstype");
    return res.data.data;
  };
  ...
  const handleConfirmApproval = async () => {
    ...
    try {
      const response = await api.post(
        `/ac/approved-by-ro/${complaintToApprove.id}`
      );
