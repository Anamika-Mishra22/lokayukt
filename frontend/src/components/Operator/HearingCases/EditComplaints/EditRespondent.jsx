 ...
 fMfLVªDV pqus
        ...
        <div>
          ...
          <select
            ...
            className={`w-full px-3 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-blue-500 ${
              formData.officer_category ? " text-[16px]" : "font-sans text-sm text-gray-500"
            }`}
          >
            <option value="" className="font-sans text-sm text-gray-500">Select Category</option>
            {categoryData?.map((cat, i) => (
              <option key={i} value={cat.name} className=" text-[16px] text-black">
                {cat.name} / {cat.name_h}
              </option>
            ))}
          </select>
