...
const AllRtiPermission = () => {
  ...
  const getSubroleRole = async () => {
    const res = await api.get("/operator/get-roles-supervisor");
  ...
  const getSavedPermissions = async () => {
    const res = await api.get(`/operator/private-file-permission/${id}`);
    return res.data.data; 
  };
  ...
  const handleSave = async () => {
    ...
    try {
      setIsSaving(true);
      await api.post("/operator/file/give-permission", payload);
