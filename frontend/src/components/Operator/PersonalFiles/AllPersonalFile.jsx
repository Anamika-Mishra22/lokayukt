...
const AllLeaveFiles = () => {
  ...
  const getAllFiles = async () => {
    const res = await api.get("/operator/get-leave-personal-details");
    return res.data;
  };
...
const approvePersonalFile = async (id) => {
  const res = await api.post(`/operator/approved-personal-file/${id}`);
  return res.data;
};
...
const approveMutation = useMutation({
  ...
  onSuccess: (data) => {
    ...
    setopenSendPoup(false);
    setSelectedFileId(null);
    queryClient.invalidateQueries(["get-leave-personal-details"]);
  },
  ...
  <div className="w-full md:w-auto flex flex-col sm:flex-row items-stretch sm:items-center gap-3">
    ...
    <button
            onClick={() => navigate("/operator/add-personal-file")}
...
>
  Send to Secretary
                    ...
                    <td className="px-6 py-4 text-right">
                      <button
                        onClick={() => navigate(`/operator/all-personal-file/${row.id}`)}
