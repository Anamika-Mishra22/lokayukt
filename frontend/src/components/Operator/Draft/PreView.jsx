...
const PreView = ({ complaintData }) => {
  ...
  const safeData = (data, fallback = "miyC/k ugha") => 
    (data !== null && data !== undefined && data !== "null" && data !== "") ? data : fallback;

  // Helper to format Date
  const formatDate = (dateString) => {
    if (!dateString) return "miyC/k ugha";
    const date = new Date(dateString);
    return isNaN(date.getTime()) ? "miyC/k ugha" : date.toLocaleDateString("hi-IN", {
      day: "2-digit",
      month: "2-digit",
      year: "numeric",
    });
    ...
    <div className="w-full h-full bg-[#FFFBF2] rounded-lg flex flex-col overflow-hidden shadow-xl border ">
      ...
      <div className="flex items-center justify-between px-6 py-4 bg-white shrink-0">
        <h2 className="text-xl font-bold text-gray-800">
          शिकायत - {safeData(data.complain_no)}
        </h2>
              ...
              <div className="flex-1">
                    ...
                    validRespondents.map((resp, index) => (
                        <div key={index} className="p-3 rounded shadow-sm border ">
                            ...
                            <div className="space-y-1 font-medium text-gray-800 text-sm">
                                ...
                                <div className="grid grid-cols-[80px_1fr]"><span className="font-semibold text-gray-900">पदनाम:</span> <span className=" kruti-input text-[15px] font-normal">{safeData(resp.designation)}</span></div>
                ...
                <div className="mt-2">
                   ...
                   <div className="pl-4 mt-0.5 font-medium">
                     <span className="kruti-input text-[20px] font-normal"> {safeData(data.delay_reason, "miyC/k ugha")} </span>
                   </div>
                </div>
            ...
            <div className="flex gap-2">
               ...
               <div className="flex-1">
                 ...
                 <div className="pl-4 mt-0.5 space-y-0.5 font-medium">
                   ...
                   <p>दिनांक: {data.challan_date || "उपलब्ध नहीं"}</p>
                   {data.challan_file && (
            ...
            <div className="flex gap-2">
                ...
                <div className="flex-1">
                  ...
                  {validSupport.length > 0 ? (
                    ...
                    <div className="pl-4 mt-0.5 font-medium kruti-input">miyC/k ugha</div>
                  )}
                </div>
            </div>
