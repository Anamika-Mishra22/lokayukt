...
const Notes = ({ complaint }) => {
  ...
  useEffect(() => {
    const fetchDocs = async () => {
      try {
        const res = await api.get(`/operator/get-letter-doc/${id}`);
  ...
  const fetchNotes = async () => {
    try {
      setLoading(true);
      const res = await api.get(`/operator/get-letter-notes/${id}`);
      if (res.data.status) {
  ...
  const handleFinalSubmit = async () => {
    ...
    try {
      const res = await api.post("/operator/add-letter-notes", payload);
