...
const Inbox = () => {
  ...
  const { data, isLoading, isError, error } = useQuery({
    ...
    queryFn: async () => {
      const response = await api.get(`/operator/all-letters?page=${page}`);
  ...
  const approveMutation = useMutation({
    mutationFn: async (id) => {
      const response = await api.post(`/operator/approved-letter-by-rk/${id}`);
      return response.data;
    },
    onSuccess: (data) => {
                          ...
                          <div className="flex gap-2 w-full sm:w-auto">
                            ...
                            {isAlreadySent ? (
                              <span className="flex-1 sm:flex-none px-2 py-1.5 bg-green-100 text-green-700 rounded-md text-[11px] font-medium whitespace-nowrap flex items-center justify-center gap-1">
                                <IoCheckmarkCircleOutline className="w-3 h-3" /> Sent to Secretary
                              </span>
                            ) : (
                              <button
