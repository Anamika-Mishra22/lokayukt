...
const ViewSend = () => {
 ...
 const getUserLabel = (option) => {
  ...
  // (यहाँ मैंने '/operator/view-letter-record' लगाया है, अगर आपकी API का नाम अलग है तो बदल लेना)
  const { data: letterData, isLoading, isError, error } = useQuery({
    queryKey: ["letter-details", id],
    queryFn: async () => {
      const res = await api.get(`/operator/view-letter-record/${id}`);
...
return raw.flat(); // ⭐ जरूरी
  ...
  const forwardMutation = useMutation({
    mutationFn: async ({ recordId, forwardTo }) => {
      ...
      const res = await api.post("/operator/forward-letter-record", {
        record_id: recordId, 
    ...
    onSuccess: (data) => {
      ...
      setTimeout(() => {
        navigate("/operator/letter-record"); // वापस लिस्ट पर भेज दें
      }, 1500);
    },
    onError: (error) => {
