...
const AllRtiFile = () => {
  ...
  const getAllFiles = async () => {
    const res = await api.get("/operator/get-leave-rti-details");
    return res.data;
  };

  const { data: allFileData, isLoading } = useQuery({
    queryKey: ["get-leave-rti-details"],
    queryFn: getAllFiles
  });


  const getRoleSubrole = async ()=>{
    const res = await api.get("/operator/get-roles-supervisors");
  ...
  const res = await api.post("/operator/access-files-permission", {
    file_id,
    user_id,
  });
  return res.data;
  ...
  const updateLeaveStatus = async ({ id, status }) => {
    const res = await api.post(`/operator/update-leave-status/${id}`, { status });
    return res.data;
  };
...
const approvePersonalFile = async (id) => {
  const res = await api.post(`/operator/approved-rti-file/${id}`);
  return res.data;
};

const approveMutation = useMutation({
  mutationFn: approvePersonalFile,

  

  onSuccess: (data) => {
    toast.dismiss();
    toast.success("File sent to Secretary");
    setopenSendPoup(false);
    setSelectedFileId(null);
    queryClient.invalidateQueries(["get-leave-rti-details"]);
  },
  ...
  const handleViewFile = async (fileId) => {
    try {
      ...
      const res = await api.get(`/operator/get-file-pdf/${fileId}`);
  ...
  <div className="w-full flex items-center gap-3">
    <div>
      <h1 className="text-xl font-bold text-gray-900">All Appeal and RTI Files</h1>
      <p className="text-sm text-gray-600">समस्त आरटीआई फाइलें</p>
    </div>
  </div>
 ...
 <button
  onClick={() => navigate("/operator/add-rti-file")}
                  ...
                  <tr key={row.id} className="hover:bg-gray-50 transition-colors">
                    ...
                    {/* <td className="px-6 py-4">
                      <div className="flex items-center gap-2">
                        ...
                        <span className="text-sm font-semibold text-gray-800 capitalize">
                          {row.user_name?.name || "उपलब्ध नहीं"}
                        </span>
                      </div>
                    </td> */}
                    ...
                    <td className="px-6 py-4">
                      <div className="flex items-center gap-3">
                        ...
                        <div>
                            <p className="text-sm kruti-input font-medium text-gray-900">{row?.title || "उपलब्ध नहीं"}</p>
                        </div>
                      </div>
                    </td>
...
>
  Send to Pio
                    ...
                    <td className="px-6 py-4 text-right">
                      <button
                        onClick={() => navigate(`/operator/all-rti-file/${row.id}`)}
  ...
  <p className="text-gray-700 text-sm">
    Are you sure you want to send this file to Secretary?
  </p>
</div>
  ...
  onClick={() => {
    ...
    approveMutation.mutate(selectedFileId);
  }}
