...
//             {complaintData.main_respondent_designation || "उपलब्ध नहीं"}
//           </p>
//         </div>
...
//                               </td>
//                               <td className="px-6 py-4 F whitespace-nowrap text-sm text-gray-700 border-r border-gray-200">
//                                 {comp.father_name || "-"}
//                               </td>
...
const ViewApprovedComplaints = () => {
  ...
  const {
    ...
    queryFn: async () => {
      try {
        const res = await api.get("/operator/get-users-sent");
        if (Array.isArray(res.data)) {
          return res.data;
        } else if (res.data && Array.isArray(res.data.data)) {
          return res.data.data;
  ...
  const forwardPhysicallyMutation = useMutation({
    mutationFn: async ({ complaintId, forwardTo }) => {
      // ---> PAYLOAD UPDATED HERE <---
      const res = await api.post("/operator/forward-physical", {
        complaint_id: complaintId,
        forward_to: forwardTo,
        target_date: targetDate,
  ...
  const handleConfirmYes = () => {
    if (confirmConfig.type === "receive") {
      ...
      if (!selectedForwardTo) {
        toast.error("Please select forward to");
        return;
      }
      forwardPhysicallyMutation.mutate({
        complaintId: id,
    ...
    : "उपलब्ध नहीं"}
            ...
            <div className="border-t p-4">
              <div className="flex flex-col sm:flex-row gap-3 justify-between">
                ...
                <div className="flex gap-2">
                  {/* 👇 YAHAN CONDITION LAGAYI HAI: Agar case_type 1 (New) hai tabhi Send button dikhega 👇 */}
                  {complaintData.case_type == 1 && (
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black bg-opacity-50 p-4">
      ...
      {confirmConfig.open && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black bg-opacity-40 p-4 overflow-y-auto">
          <div className="bg-white rounded-lg shadow-lg w-full max-w-md p-5 relative animate-in fade-in zoom-in duration-200 my-auto">
            <button
              onClick={handleConfirmNo}
              disabled={
                markAsReceivedMutation.isPending ||
            ...
            {/* ---> MODIFIED FORWARD MODAL CONTENT BEGINS HERE <--- */}
            {confirmConfig.type === "forward" && (
              <>
                {/* 1. Forward To (Select User) */}
                <div className="mb-4">
                  <label className="block text-sm font-medium text-gray-700 mb-2">
                    Forward To <span className="text-red-500">*</span>
                  </label>
                  ...
                  {isLoadingOptions || isFetchingOptions ? (
                    ...
                    >
                     ...
                     {Array.isArray(forwardOptionsData) &&
                      forwardOptionsData.length > 0 ? (
                        forwardOptionsData.flat().map((option) => (
                          <option key={option.id} value={option.id}>
                            {option.name || option.user_name || `User ${option.id}`}
                            {option.role?.label === "Supervisor"
                ...
                {/* 4. OTP (Shown only if a user is selected) */}
                {/* {selectedForwardTo && (
                  <div className="mb-5">
                    <label className="block text-sm font-medium text-gray-700 mb-2">
                    ...
                    />
                    {fieldErrors?.otp && (
                      <p className="text-red-500 text-[13px] mt-1 font-medium">{fieldErrors.otp[0]}</p>
                    )}
                  </div>
                )} */}
              </>
            )}
            {/* ---> MODIFIED FORWARD MODAL CONTENT ENDS HERE <--- */}
                ...
                complaintData.complainants.length > 0 ? (
                  <div className="overflow-x-auto border border-gray-200 rounded-lg shadow-sm">
                    <table className="min-w-full divide-y divide-gray-200">
                      <thead className="bg-gray-100">
                        <tr>
                          <th className="px-6 py-4 text-left text-[14px] font-bold text-gray-600 uppercase tracking-wider whitespace-nowrap border-r border-gray-200">
                            क्र. सं.
                          </th>
                          <th className="px-6 py-4 text-left text-[14px] font-bold text-gray-600 uppercase tracking-wider whitespace-nowrap border-r border-gray-200">
                            नाम
