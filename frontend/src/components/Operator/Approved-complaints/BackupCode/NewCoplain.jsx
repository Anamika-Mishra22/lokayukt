...
//   // Main search query (used for both frontend filtering and backend API search)
...
//   // ----------------------------------------------------
//   // 1. OLD API: Initial Load All Complaints (No Changes)
//   // ----------------------------------------------------
...
//   // ----------------------------------------------------
//   // 2. NEW API: Search by Complaint No (from Main Input) and Year (GET Request)
//   // ----------------------------------------------------
//   const handleSearch = async () => {
//     // Agar dono input khali hain toh wapas pura data dikha do
//     if (!searchQuery && !searchYear) {
//       let originalData = [];
//       if (data && Array.isArray(data)) originalData = data;
//       else if (data?.data && Array.isArray(data.data)) originalData = data.data;
      
//       setAllComplaints(originalData);
//       setCurrentPage(1);
//       return;
//     }
...
//     try {
//       setIsSearching(true);
//       // ✅ Payload bhej rahe hain GET API me (params ke through)
//       const payload = {};
//       if (searchQuery) payload.complain_no = searchQuery; // Main search input se complain_no
//       if (searchYear) payload.year = searchYear;

//       // ✅ POST ki jagah GET ka use kiya gaya hai aur data 'params' me bheja hai
...
//         searchData = [response.data.data]; // For single object
//       }

//       setAllComplaints(searchData);
//       setCurrentPage(1);
      
//       if(searchData.length === 0) {
//          toast.info("No complaints found for this search.");
//       }
...
//     // Frontend Filtering with the main search bar
//     if (searchQuery.trim() !== "") {
...
//   // ✅ Generate Year Options down to 1997
...
//             {/* ✅ Main Search Input (Used for API Search too) */}
...
//             {/* ✅ Responsive Wrapper Class (flex-wrap) */}
//             <div className="flex flex-wrap items-center gap-2 text-xs w-full">
//                 <select
//                   className="border border-gray-300 px-2 py-1.5 rounded-md focus:outline-none focus:ring-1 focus:ring-blue-500 text-xs min-w-[120px] flex-grow sm:flex-grow-0"
...
//                   className={`border border-gray-300 px-2 py-1.5 rounded-md focus:outline-none focus:ring-2 focus:ring-blue-500 bg-white min-w-[120px] flex-grow sm:flex-grow-0 ${
...
//                 <select
//                   className="border border-gray-300 px-2 py-1.5 rounded-md text-xs min-w-[120px] flex-grow sm:flex-grow-0"
...
//                 {/* ✅ Year Dropdown with data down to 1997 */}
//                 <select
//                   className="border border-gray-300 px-2 py-1.5 rounded-md text-xs min-w-[100px] flex-grow sm:flex-grow-0 focus:outline-none focus:ring-1 focus:ring-blue-500"
//                   value={searchYear}
...
//                 {/* ✅ API Call Search Button */}
//                 <button
//                   onClick={handleSearch}
//                   disabled={isSearching}
//                   className="px-4 py-1.5 bg-blue-600 text-white rounded-md text-xs hover:bg-blue-700 transition-colors disabled:opacity-50 min-w-[80px] flex-grow sm:flex-grow-0"
//                 >
//                   {isSearching ? "Searching..." : "Search"}
//                 </button>

//                 {/* Sort Dropdown Pushed to right via ml-auto on large screens, standard wrap on small */}
...
//                   <select
//                     className="border border-gray-300 px-2 py-1.5 rounded-md focus:outline-none focus:ring-1 focus:ring-blue-500 text-xs w-full sm:w-auto"
//                     value={sortOrder}
...
//                 <h1 className="text-gray-600">{isSearching ? "Searching Data..." : "Loading..."}</h1>
//               </div>
//             )
//             :
...
//                             :</span>
//                           <span className="ml-1">
//                             {complaint.category || "उपलब्ध नहीं"}
//                           </span>
//                         </div>
