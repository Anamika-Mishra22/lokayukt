...
const AllComplaints = () => {
  ...
  // New state for Tabs
  const [activeTab, setActiveTab] = useState("normal"); // "normal" | "dispatched"
  ...
  // Dynamically change API endpoint based on selected tab
  const getAllComplaints = async () => {
    const endpoint =
      activeTab === "dispatched"
        ? "/operator/all-complaints-dispatch"
        : "/operator/all-complaints";
    const res = await api.get(endpoint);
    return res.data;
  };

  const { data, isLoading, isError, error, refetch } = useQuery({
    queryKey: ["complaints", location.pathname, activeTab], // Add activeTab to queryKey
  ...
  useEffect(() => {
    if (data?.data && Array.isArray(data.data)) {
      ...
      // Clear data if no data array is found (e.g. switching tabs with no results)
      setAllComplaints([]);
      setFilteredComplaints([]);
    }
  }, [data, sortOrder, activeTab]); // Added activeTab dependency
  ...
  useEffect(() => {
    ...
    if (selectedFeeStatus !== "") {
      filtered = filtered.filter(
        (complaint) =>
          complaint.fee_exempted?.toString() === selectedFeeStatus
      );
    }
