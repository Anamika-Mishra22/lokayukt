...
setTimeout(() => {
  ...
  useEffect(() => {
    const fetchDocs = async () => {
      try {
        const res = await api.get(`/st/get-document/${complaint?.id}`);
  ...
  const fetchNotes = async () => {
    ...
    try {
      setLoading(true);
      const res = await api.get(`/st/get-notes/${complaint.id}`);
      if (res.data.status) {
  ...
  const fetchPdfPath = async (filename) => {
    try {
      const res = await api.get(`/st/get-file-preview/${complaint.id}`);
  ...
  const handleEditClick = async (noteId) => {
    try {
      setLoading(true);
      const res = await api.get(`/st/note/${noteId}`);
  ...
  "pdf-content-div"
    ...
    try {
      let res;
      if (editNoteId) {
        res = await api.put(`/st/notes/${editNoteId}`, payload);
      } else {
        res = await api.post("/st/add-notes", payload);
      }
...
`}</style>
  ...
  <div className="flex pt-4 mt-auto pdf-bottom-signature w-full">
    <div className="text-start text-xs text-gray-600">
      <p>{name} (ST) {new Date().toLocaleDateString("en-IN")}</p>
    </div>
  </div>
</div>
