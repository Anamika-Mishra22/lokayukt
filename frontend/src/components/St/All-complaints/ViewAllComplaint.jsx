...
const getDisplayLabel = (option) => {
  ...
  let subRole =
    option.sub_role?.label ||
    option.sub_role?.name ||
    option.subrole_name ||
  ...
  // 🔥 PS Logic
  if (option?.role_name?.toLowerCase() === "ps") {
    subRole = option.subrole_name
      ? "Additional Private Secretary"
      : "Private Secretary";
  }

  // 🔥 APS Logic
  if (option?.role_name?.toLowerCase() === "aps") {
    subRole = ", Private Secretary";
  }
...
const ViewAllComplaint = () => {
  ...
  const {
    ...
    queryFn: async () => {
      const res = await api.get(`/st/view-complaint/${id}`);
  ...
  const {
    ...
    queryFn: async () => {
      try {
        // 1️ Send To My Pool → API: /st/get-users
        if (forwardType === "self") {
          const res = await api.get("/st/get-users");
          return res.data?.data || res.data || [];
        }

        // 2️ Send To Other Pool → API: /st/get-lokayukt-uplokayukt
        const res = await api.get("/st/get-lokayukt-uplokayukt");
...
return flatList.filter(
  ...
  const dispose = useMutation({
    mutationFn: async ({ complaintId, remarkData }) => {
      // ध्यान दें: यहाँ मैंने API endpoint /st/dispose-complain कर दिया है
      const res = await api.post(`/st/dispose-complain/${id}`, {
  ...
  mutationFn: async ({ complaintId }) => {
    const res = await api.post(
      `/st/reject-complaint-by-st/${complaintId}`
    );
    return res.data;
  },
  onSuccess: () => {
  ...
  const pullBackMutation = useMutation({
    mutationFn: async ({ complaintId }) => {
      const res = await api.post(`/st/pull-back-by-st/${complaintId}`);
      return res.data;
    },
    onSuccess: (data) => {
  ...
  const assignToSelfMutation = useMutation({
    mutationFn: async ({ complaintId }) => {
      const res = await api.post(`/st/assign-by-st/${complaintId}`);
      return res.data;
    },
    onSuccess: (data) => {
  ...
  const returnWithRemarksMutation = useMutation({
    mutationFn: async ({ complaintId, remarkData }) => {
      const res = await api.post(`/st/return-complain-by-st/${complaintId}`, {
        remark: remarkData,
      });
      return res.data;
    },
  ...
  onError: (error) => {
      // ✅ BACKEND ERRORS YAHAN CATCH HONGE
      const resData = error?.response?.data;
      if (resData?.errors) {
        setFieldErrors(resData.errors);
 ...
 const forwardComplaintMutation = useMutation({
    mutationFn: async ({ complaintId, forwardTo, remarkData, otpData }) => {
      const res = await api.post(`/st/forward-complain-by-st/${complaintId}`, {
...
<div className="hidden md:block">
  ...
  <div className="flex flex-col gap-3 text-sm text-slate-700 mt-1 pl-1">
    ...
    <p>
      ...
      Complaint send back to Hon’ble Lokayukt / Hon’ble UpLokayukt .
    </p>

    <p>
      <span className="font-semibold text-slate-900">Take File in Hand</span> 
      <span className="mx-2 text-slate-400">—</span> 
      Assign the complaint to yourself for further action.
    </p>
    ...
    <p>
      ...
      <span className="mx-2 text-slate-400">—</span> 
        Reject button for reject complaint.
    </p>
    ...
    <p>
      ...
      <span className="mx-2 text-slate-400">—</span> 
      Return the file with comments or objections.
    </p>
...
{/* SEND → OTP Input & Button */}
            {confirmConfig.type === "forward" && selectedForwardTo && (
              <div className="mb-5 mt-3">
                ...
                {fieldErrors?.otp && (
                    <p className="text-red-500 text-[13px] font-medium">{fieldErrors.otp[0]}</p>
                  )}
              </div>
            )}
                    ...
                    >
                      ...
                      <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                        ...
                        <div className="p-3 bg-gray-50 rounded">
                          ...
                          <p className="text-gray-800">
                            {item.support_address || "उपलब्ध नहीं"}
                          </p>
                        </div>
                      </div>
                    </div>
                    ...
                    >
                      ...
                      <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                        ...
                        <div className="p-3 bg-gray-50 rounded">
                          ...
                          <p className="text-gray-800">
                            {item.witness_address || "उपलब्ध नहीं"}
                          </p>
                        </div>
                      </div>
                    </div>
