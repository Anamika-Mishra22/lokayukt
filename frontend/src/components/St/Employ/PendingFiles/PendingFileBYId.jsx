...
import MovementHistory from "../../Employ/SubModule/MovementHistory"
...
const PendingFileBYId = () => {
  ...
  const { data: response, isLoading } = useQuery({
    ...
    queryFn: async () => {
      const res = await api.get(`/st/personal-file-list/${id}`);
      return res.data;
    },
    enabled: !!id
  });
  ...
  const { data: roleSubrole, isLoading: isRolesLoading } = useQuery({
    ...
    queryFn: async () => {
      const res = await api.get("/st/get-roles-supervisors");
  ...
  const sendPermissionMutation = useMutation({
    mutationFn: async ({ file_id, to_user_id }) => {
      const res = await api.post("/st/personal-file-send", {
        file_id,
        to_user_id,
      });
      return res.data;
