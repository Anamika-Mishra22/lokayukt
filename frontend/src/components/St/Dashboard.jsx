...
const [isLoading, setIsLoading] = useState(false);
  ...
  const fetchWeeklyData = async () => {
    try {
      ...
      const response = await api.get('/ac/getWeeklyGraph');
  ...
  const fetchDashboardData = async (monthParam) => {
    try {
      ...
      const dashResponse = await api.get(`/ac/dashboard/${monthParam}`);
      ...
      const monthlyResponse = await api.get('/ac/montly-complaint');
      ...
      const statusResponse = await api.get('/ac/status-distribution');
      ...
      const deptResponse = await api.get('/ac/department-wise-complaint');
      ...
      // 5. District-wise API
      const districtResponse = await api.get('/ac/district-wise-company-type');
...
>
 ...
 <div className="grid grid-cols-1 sm:grid-cols-6 md:grid-cols-2 lg:grid-cols-4 xl:grid-cols-6 gap-4">
        ...
        <div 
          onClick={()=>{
          navigate("/ac/approved-complaints")
        }}
        ...
        <div
        //   onClick={()=>{
        //   navigate("/ac/pending-complaints")
        // }}
