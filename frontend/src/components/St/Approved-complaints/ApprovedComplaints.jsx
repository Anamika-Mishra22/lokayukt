...
//     const resNew = await api.get("/st/all-approved-complaints");
...
//  console.log(res.roleParent)
  ...
  const getAllComplaints = async () => {
    const res = await api.get("/st/all-approved-complaints");
...
const handleSendToUPLokayukt = async () => {
  ...
  try {
    ...
    await api.post(
      `/st/forward-to-uplokayukt/${complaintToApprove.id}`,
      {
  ...
  const getDistrict = async () => {
    const res = await api.get("/st/all-district");
  ...
  const getComplaintTypes = async () => {
    const res = await api.get("/st/complainstype");
    return res.data.data;
  };
  ...
  const handleConfirmApproval = async () => {
    ...
    try {
      const response = await api.post(
        `/st/approved-by-ro/${complaintToApprove.id}`
      );
