...
const Login = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      // const macRes = await axios.get("http://127.0.0.1:5000/mac");
      // const macAddress = macRes.data.mac;
      ...
      if (response.data.status === 'success') {
        ...
        setTimeout(() => {
          if (userRole === "admin") {
            ...
            Cookies.set('subrole', response.data.data.user.subrole.name);
            Cookies.set('subroleName', response.data.data.user.subrole.label);
            // Cookies.set('UserN', response.data.data.user.name);
          ...
          // }
            ...
            window.open("/employee/view-files", "_self");
          }
            ...
            Cookies.set('subrole_lable', response.data?.data?.user?.subrole?.label);  
            ...
            Cookies.set('name', response?.data?.data?.user?.name);
            Cookies.set('subroleName', response?.data?.data?.user?.role?.label);
            ...
            Cookies.set("UserID", response.data.data.user.id);
            Cookies.set('name', response?.data?.data?.user?.name);
            Cookies.set('subRole', response?.data?.data?.user?.role?.label);

          }
          else if (userRole === "assistant-clerk") {
            window.open("/main-dashboard", "_self");
