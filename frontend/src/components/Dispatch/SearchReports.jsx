...
const ForwardModal = ({ isOpen, onClose, complaintId, onSubmit }) => {
  ...
  const fetchInvestigationData = async () => {
    ...
    try {
      const response = await api.get(`/dispatch/request-list-cio/${complaintId}`);
      if (response.data.status === true && Array.isArray(response.data.data)) {
        const processedData = response.data.data.map((item) => {
          let officerName = "उपलब्ध नहीं";
          let role = "उपलब्ध नहीं";
          ...
          return {
            name: officerName,
            role: role,
            remark: item.remarks || "उपलब्ध नहीं",
            targetDate: item.target_date || "उपलब्ध नहीं",
  ...
  const fetchUsersData = async () => {
    ...
    try {
      const response = await api.get("/dispatch/get-users");
  ...
  const fetchReportData = async () => {
    ...
    try {
      const response = await api.get(`/dispatch/request-list/${complaintId}`);
      if (response.data.status === true && Array.isArray(response.data.data)) {
        ...
        const processedData = response.data.data.map((item) => {
          ...
          return {
            ...
            role: role,
            remark: item.remarks || "उपलब्ध नहीं",
            entryDate: item.created_at || "उपलब्ध नहीं"
          };
        });
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const response = await api.post(`/dispatch/request-report/${complaintId}`, forward);
...
const SearchReports = () => {
    ...
    try {
      ...
      const wsData = [
        ...
        ...filteredResults.map((item, index) => [
          ...
          item.application_no || "उपलब्ध नहीं", 
          item.name || "उपलब्ध नहीं",
          item.officer_name || "उपलब्ध नहीं",
          item.department_name || "उपलब्ध नहीं",
          item.district_name || "उपलब्ध नहीं",
          item.complaintype_name || "उपलब्ध नहीं",
          item.status || "उपलब्ध नहीं",
          item.created_at || "उपलब्ध नहीं"
        ])
      ];
  ...
  useEffect(() => {
    const fetchInitialData = async () => {
      ...
      try {
        const districtsResponse = await api.get("/dispatch/all-district");
        if (districtsResponse.data.status === "success") {
          const districtsArray = ensureArray(districtsResponse.data.data);
          setDistricts(districtsArray);
        }

        const reportsResponse = await api.get("/dispatch/complain-report");
        ...
        try {
          const overallResponse = await api.get("/dispatch/all-complains");
        ...
        try {
          const districtWiseResponse = await api.get("/dispatch/district-wise-complaint");
        ...
        try {
          const departmentWiseResponse = await api.get("/dispatch/department-wise-complaint");
        ...
        try {
          const monthlyTrendsResponse = await api.get("/dispatch/montly-trends");
        ...
        try {
          const avgProcessingResponse = await api.get("/dispatch/detail-by-complaintype");
        ...
        try {
          const complianceReportResponse = await api.get("/dispatch/compliance-report");
  ...
  const handleSearch = async () => {
    setIsSearching(true);
    try {
      const response = await api.get("/dispatch/complain-report");
  ...
  const calculateOverallAverage = () => {
    if (!avgProcessingTimes || !Array.isArray(avgProcessingTimes)) return "उपलब्ध नहीं";
    const validTimes = avgProcessingTimes.filter((item) => item.avgdays !== null && !isNaN(parseFloat(item.avgdays)));
    if (validTimes.length === 0) return "उपलब्ध नहीं";
                                ...
                                paginatedResults.map((result, index) => (
                                  <tr key={result.id} className="hover:bg-gray-50">
                                    <td className="py-2 px-2 sm:px-3 font-medium text-gray-700 hover:text-blue-800 hover:underline cursor-pointer" 
                                        onClick={() => navigate(`/dispatch/search-reports/view/${result.id}`)}>
                                      {result.complain_no || result.application_no || "उपलब्ध नहीं"}
                                    </td>
                                    <td className="py-2 px-2 sm:px-3 text-gray-700">
                                      {result.name || "उपलब्ध नहीं"}
                                    </td>
                                    <td className="py-2 px-2 sm:px-3 text-gray-700 hidden lg:table-cell">
                                      {result.designation_name || "उपलब्ध नहीं"}
                                    </td>
                                    <td className="py-2 px-2 sm:px-3 text-gray-700 hidden lg:table-cell">
                                      {result.department_name || "उपलब्ध नहीं"}
                                    </td>
                                    <td className="py-2 px-2 sm:px-3 text-gray-700">
                                      <span    
                                      >
                                        {result.district_name || "उपलब्ध नहीं"}
                                      </span>
                                    </td>
                                    <td className="py-2 px-2 sm:px-3">
                                      ...
                                      >
                                        {result.complaintype_name || "उपलब्ध नहीं"}
                                      </span>
                                    </td>
                                    <td className="py-2 px-2 sm:px-3">
                                      <span
                              ...
                              {avgProcessingTimes.map((item, idx) => (
                                <div key={idx} className="flex justify-between">
                                  ...
                                  <span className="font-medium text-gray-900">
                                    {item.avgdays !== null ? `${item.avgdays} days` : "उपलब्ध नहीं"}
                                  </span>
                                </div>
                              ))}
