...
const EditUserManagement = () => {
  ...
  useEffect(() => {
    const fetchAllData = async () => {
      try {
        ...
        const userResponse = await api.get(`/lokayukt/edit-users/${id}`);
        if (userResponse.data.status === true) {
          ...
          navigate('/lokayukt/user-management'); // Redirect if user not found
        }
  ...
  useEffect(() => {
    async function fetchRoles() {
      try {
        const res = await api.get("/lokayukt/get-roles");
  ...
  useEffect(() => {
    async function fetchSubRoles() {
      ...
      try {
        const res = await api.get(`/lokayukt/get-sub-roles/${formData.role_id}`);
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (response.data.status === true) {
        ...
        setTimeout(() => {
          navigate("/lokayukt/user-management")
        }, 2000);
      }
    } catch (error) {
        ...
        <div className="space-y-4 sm:space-y-6">
          ...
          <div className="bg-white p-4 sm:p-6 rounded-lg border border-gray-200">
            <div className="flex justify-end gap-3">
              <button
                type="button"
                onClick={() => navigate("/lokayukt/user-management")}
