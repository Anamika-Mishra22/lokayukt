...
const PendingFileBYId = () => {
  ...
  const { data: response, isLoading } = useQuery({
    ...
    queryFn: async () => {
      const res = await api.get(`/dispatch/personal-file-list/${id}`);
      return res.data;
    },
    enabled: !!id
  });
  ...
  const sendPermissionMutation = useMutation({
    mutationFn: async ({ file_id, to_user_id }) => {
      const res = await api.post("/dispatch/forward-file-send", {
        file_id,
        to_user_id,
      });
      return res.data;
...
if (isLoading)
  ...
  <div className="relative w-full">
    ...
    {/* Dropdown Button (Jo Select Box jaisa dikhega) */}
    <div
    ...
    {isDropdownOpen && (
      <div className="absolute z-50 w-full mt-1 bg-white border rounded-md shadow-lg">
        ...
        {/* EXACTLY Dropdown ke andar Search Box */}
        <div className="p-2 border-b bg-gray-50">
          <input
            ...
            autoFocus // Open hote hi direct type kar sakte hain
          />
        </div>
              ...
              return (
                <li
                  ...
                  onClick={() => {
                    ...
                    setIsDropdownOpen(false); // Select karne par band kar do
                    setSearchTerm(""); // Search clear kar do
                  }}
