...
const EmployScaneLetter = () => {
  ...
  const getAllComplainsID = async () => {
    const res = await api.get("/dispatch/personal-file-id");
    return res.data;
  };

  const { data: complainIdsData } = useQuery({
    queryKey: ["personal-file-id"],
    queryFn: getAllComplainsID,
  });

  const complainList = Object.entries(complainIdsData?.data || {}).map(
  ([id, value]) => ({
    id,
    name: value,
  })
    ...
    if (complainList.length > 0) {
      if (searchCase.trim() === "") {
        ...
        const filtered = complainList.filter((item) =>
          String(item).toLowerCase().includes(searchCase.toLowerCase())
        );
  ...
  const getAllDispatchLetters = async () => {
    const res = await api.get("/dispatch/all-dispatch-personal-letters");
    return res.data;
  };
 ...
 const submitScaneData = async (e) => {
  ...
  try {
    const res = await api.post("/dispatch/add-personal-dispatch", payload, {
      headers: {
...
const makeFileUrl = (filePath) => {
          ...
          <table className="w-full text-sm text-left text-gray-500 min-w-[800px]">
            <thead className="text-xs text-gray-700 uppercase bg-gray-50 border-b">
              <tr>
                ...
                <th className="px-6 py-3 whitespace-nowrap">File No.</th>
                ...
                lettersList.map((row) => (
                  ...
                  >
                    ...
                    {/* <td className="px-6 py-4 whitespace-nowrap text-right flex justify-end items-center gap-3">
                      ...
                      <td>
                       <button
                      onClick={() => handleViewPdf(row.file, row.file_id)}
  ...
  className={`kruti-input w-full px-3 py-2 bg-gray-50 border rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-blue-500 focus:bg-white transition-all text-gray-600 ${
    errors.file_id ? "border-red-300" : "border-gray-200"
  }`}
>
  ...
  {filteredComplainList.length > 0 ? (
    ...
    <option value="" disabled>
      No cases found
              ...
              <div>
  <label className="block text-sm font-medium text-gray-700 mb-1">
    Letter No / पत्र संख्या <span className="text-red-500">*</span>
  </label>
...
<input
  ...
  className={`w-full px-3 py-2 bg-gray-50 border rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-blue-500 ${
    errors.letter_no ? "border-red-300" : "border-gray-200"
  }`}
/>
...
export default EmployScaneLetter;