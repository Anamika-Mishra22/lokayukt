...
const ViewInbox = () => {
  ...
  // (यहाँ मैंने '/dispatch/view-letter-record' लगाया है, अगर आपकी API का नाम अलग है तो बदल लेना)
  const { data: letterData, isLoading, isError, error } = useQuery({
    queryKey: ["letter-details", id],
    queryFn: async () => {
      const res = await api.get(`/dispatch/view-letter-record/${id}`);
...
return raw.flat(); // ⭐ जरूरी
  ...
  const forwardMutation = useMutation({
    mutationFn: async ({ recordId, forwardTo }) => {
      // 🟢 पेलोड में complaint_id की जगह record_id भेज रहे हैं
      const res = await api.post("/dispatch/forward-letter-record", {
        record_id: recordId, 
    ...
    onSuccess: (data) => {
      ...
      setTimeout(() => {
        navigate("/dispatch/letter-record"); // वापस लिस्ट पर भेज दें
      }, 1500);
    },
    onError: (error) => {
        ...
        <div className="p-4 md:p-6 border-b">
          <div className="flex justify-between items-start mb-3">
            <h2 className="text-xl font-semibold text-gray-800 flex items-center gap-2">
              Letter No. <span className="text-blue-700">{letterData?.letter_no || "उपलब्ध नहीं"}</span>
            </h2>
            <button
              onClick={() => navigate(-1)}
