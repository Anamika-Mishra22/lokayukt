...
const AllRtiPermission = () => {
  ...
  const getSavedPermissions = async () => {
    const res = await api.get(`/dispatch/private-file-permission/${id}`);
    return res.data.data; 
  };
  ...
  const handleUpdateAuthority = async () => {
    ...
    try {
      ...
      const response = await api.post("/dispatch/update-rti-user-id", payload);
  ...
  const handleSave = async () => {
    ...
    try {
      setIsSaving(true);
      await api.post("/dispatch/file/give-permission", payload);
              ...
              <div className="space-y-4">
                <div>
                  ...
                  <select
                    ...
                    className={`w-full px-3 py-2 border rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-300 bg-white text-sm ${
...
export default AllRtiPermission;