...
const ViewRtiFile = () => {
  ...
  const { data: response, isLoading } = useQuery({
    queryKey: ["rti-file-list", id],
    queryFn: async () => {
      const res = await api.get(`/dispatch/get-leave-rti-details/${id}`);
      return res.data;
    },
    enabled: !!id
  });
  ...
  const { data: roleSubrole, isLoading: isRolesLoading } = useQuery({
    ...
    queryFn: async () => {
      const res = await api.get("/dispatch/get-roles-supervisors");
  ...
  const sendPermissionMutation = useMutation({
    mutationFn: async ({ file_id, to_user_id }) => {
      const res = await api.post("/dispatch/rti-file-send", {
        file_id,
        to_user_id,
      });
      return res.data;
    ...
    onSuccess: (data) => {
      ...
      queryClient.invalidateQueries({ queryKey: ["rti-file-list", id] });
    },
    onError: (err) => {
...
if (isLoading)
  ...
  <div className="relative w-full">
    ...
    {/* Dropdown Button (Select Box jaisa) */}
    <div
      ...
      <span className="truncate">
        {isRolesLoading ? (
          ...
          (() => {
            ...
            return `${sel.name} ${sel.role?.label ? `/ (${sel.role.label})` : ""}`;
          })()
        ) : (
          "Select Authority"
        )}
    ...
    {isDropdownOpen && (
      <div className="absolute z-50 w-full mt-1 bg-white border rounded-md shadow-lg">
        ...
        <ul className="max-h-48 overflow-y-auto">
          {roleSubrole
            ?.filter((item) => {
              // Search logic matching your format
              const searchString = `${item.name} ${item.role?.label || ""}`.toLowerCase();
            ...
            .map((item) => (
              <li
                ...
                onClick={() => {
                  ...
                  setIsDropdownOpen(false); // Select karne par band ho jayega
              ...
              >
                {item.name} {item.role?.label ? `/ (${item.role.label})` : ""}
              </li>
            ))}

          {/* Agar search me kuch na mile */}
          {roleSubrole?.filter(item => {
            return `${item.name} ${item.role?.label || ""}`.toLowerCase().includes(searchTerm.toLowerCase());
          }).length === 0 && (
