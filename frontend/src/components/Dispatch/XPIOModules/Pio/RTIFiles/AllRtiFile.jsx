...
const AllRtiFile = () => {
  ...
  const getAllFiles = async () => {
    const res = await api.get("/dispatch/get-user-rti-details");
    return res.data;
  };
  ...
  const getRoleSubrole = async ()=>{
    const res = await api.get("/dispatch/get-roles-supervisor");
  ...
  const res = await api.post("/dispatch/access-files-permission", {
    file_id,
    user_id,
  });
  return res.data;
  ...
  const updateLeaveStatus = async ({ id, status }) => {
    const res = await api.post(`/dispatch/update-leave-status/${id}`, { status });
    return res.data;
  };
...
<p className="text-sm text-gray-600">समस्त आरटीआई फाइलें</p>
  ...
  <div className="w-full md:w-auto flex flex-col sm:flex-row items-stretch sm:items-center gap-3">
    ...
    {/* <button
            onClick={() => navigate("/employee/add-rti-file")}
...
{/*                     
                    ...
                    <td className="px-6 py-4 text-right">
                      <button
                        onClick={() => navigate(`/pio/all-rti-file/${row.id}`)}
