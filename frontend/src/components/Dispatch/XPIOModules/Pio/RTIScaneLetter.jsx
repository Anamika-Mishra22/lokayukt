...
const RTIScaneLetter = () => {
  ...
  const getAllComplainsID = async () => {
    const res = await api.get("/dispatch/all-rti-letter-ids");
    return res.data;
  };
  ...
  item.name.toLowerCase().includes(searchCase.toLowerCase())
);
  ...
  const getAllDispatchLetters = async () => {
    const res = await api.get("/dispatch/all-dispatch-rti-letters");
    return res.data;
  };
 ...
 const submitScaneData = async (e) => {
  ...
  try {
    const res = await api.post("/dispatch/add-rti-dispatch", payload, {
      headers: {
