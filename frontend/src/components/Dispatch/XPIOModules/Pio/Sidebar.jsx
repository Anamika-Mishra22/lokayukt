...
const Sidebar = ({ isMobileMenuOpen, toggleMobileMenu, isCollapsed }) => {
  ...
  const isFilesActive = (path) => {
    return location.pathname.startsWith("/pio/view-files") ||
           location.pathname.startsWith("/pio/add-files");
  };

  const isFilesActivee = (path) => {
    return location.pathname.startsWith("/pio/view-personal-files") ||
           location.pathname.startsWith("/pio/add-personal-files");
  };

  const isFilesActiveee = (path) => {
    return location.pathname.startsWith("/pio/view-pending-files") 
          //  location.pathname.startsWith("/pio/add-pending-files");
  };
...
{/* 
  ...
  className={`flex items-center gap-3 px-3 py-2 rounded-lg text-sm font-medium transition
    ${
      location.pathname.includes("/pio/all-rti-file") ||
      location.pathname.includes("/pio/add-rti-file")
        ? "bg-blue-600 text-white shadow-md"
        : "text-gray-700 hover:bg-gray-200"
    }
    ${isCollapsed && !isMobile ? "justify-center" : ""}
...
>
  <TbFileSearch />
  {(!isCollapsed || isMobile) && "Appeal and RTI Files"}
</Link>
...
{/*            
                                                                ...
                                                                >
                                                                  <div className="flex items-center gap-3">
                                                                    ...
                                                                    {(isMobile || !isCollapsed) && <span>Leave file approved</span>}
                                                                  </div>
                                                                  {isActive("/leave-file-approved") && (isMobile) && (
