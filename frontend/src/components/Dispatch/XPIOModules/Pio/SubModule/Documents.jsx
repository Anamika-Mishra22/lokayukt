...
const handlePrint = async () => {
  try {
    await api.post("/dispatch/user-complaint-activity-log", { 
      complain_id: complaint.id,
      user_id: Cookies.get("id") || "",
      user_name: Cookies.get("name") || "",
      file_name: viewedFileName,
      type: "PRINTED",
      file_path: pdfViewUrl
    });
  } catch (error) {
  ...
  const handleViewPdf = async (filename, title) => {
    try {
      setLoadingDoc(filename);
      setViewedFileName(title);
  ...
  const handleSubmitDocument = async () => {
    ...
    try {
      ...
      await api.post("/dispatch/upload-document", formData, {
      ...
      {pdfViewUrl && (
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4">
          <div className="bg-white rounded-lg w-full max-w-5xl h-[90vh] flex flex-col">
            ...
            <iframe
              id="pdf-iframe" 
              src={`${pdfViewUrl}#toolbar=0&zoom=page-width`}
