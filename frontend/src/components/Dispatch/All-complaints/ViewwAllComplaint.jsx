...
const getDisplayLabel = (option) => {
  ...
  if (option?.role_name?.toLowerCase() === "aps") {
    subRole = "Assistant Private Secretary";
  }
...
const forwardComplaintMutation = useMutation({
  mutationFn: async ({ complaintId, forwardTo, targetDate, assignedDate, sentThroughRk, otpData }) => { 
    const res = await api.post(`/dispatch/forward-by-dispatch/${complaintId}`, {
      forward_to: forwardTo,
      target_date: targetDate,
      assigned_date: assignedDate,
      sent_through_rk: sentThroughRk ? 1 : 0,
      otp: otpData
    });
    return res.data;
  },
  onSuccess: (data) => {
    ...
    setTimeout(()=>{
      navigate("/dispatch/all-complaints");
  ...
  <div>
    ...
    <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
      ...
      <div>
        ...
        <p className="text-gray-800 text-sm">
          {capitalizeFirstLetter(complaintData.main_respondant_district) || "उपलब्ध नहीं"}
        </p>
      </div>
    </div>
  </div>
  ...
  <div>
    ...
    : "उपलब्ध नहीं"}
              ...
              {/* <div className="flex flex-wrap gap-2 mb-4">
                ...
                >
                  शुल्क का प्रकार:{" "}
                  {complaintData.fee_exempted == 0 ? "Exempted" : complaintData.fee_exempted == 2 ? "Partial" : complaintData.fee_exempted == 1 ? "Paid" : "उपलब्ध नहीं"}
                </span>
...
{confirmConfig.type === "forward" && (
  <label className="flex items-center gap-2 cursor-pointer mt-2 mb-4">
    ...
    />
    <span className="text-sm font-medium text-gray-700">Send through RC</span>
  </label>
)}
