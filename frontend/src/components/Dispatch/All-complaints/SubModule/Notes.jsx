...
setTimeout(() => {
  ...
  const fetchNotes = async () => {
    ...
    try {
      setLoading(true);
      const res = await api.get(`/dispatch/get-notes/${complaint.id}`);
      if (res.data.status) {
  ...
  const handleEditClick = async (noteId) => {
    try {
      setLoading(true);
      const res = await api.get(`/dispatch/note/${noteId}`);
  ...
  "pdf-content-div"
    ...
    try {
      let res;
      if (editNoteId) {
        res = await api.put(`/dispatch/notes/${editNoteId}`, payload);
      } else {
        res = await api.post("/dispatch/add-notes", payload);
      }
...
`}</style>
  ...
  <div className="flex pt-4 mt-auto pdf-bottom-signature w-full">
    <div className="text-start text-xs text-gray-600">
      <p>{name} (DISPATCH) {new Date().toLocaleDateString("en-IN")}</p>
    </div>
  </div>
</div>
    ...
    flexDirection: 'column'
            ...
            >
              ...
              <div className="flex pt-4 mt-auto pdf-bottom-signature w-full">
                <div className="text-start text-xs text-gray-600">
                  <p>
                   {printNoteData?.forward_by_user?.name}
{" "}
