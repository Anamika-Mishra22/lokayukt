...
const AllLeaveFiles = () => {
  ...
  // 👇 YAHAN NAYA STATE ADD KIYA HAI POPUP KE LIYE 👇
  ...
  const getAllFiles = async () => {
    const res = await api.get("/supervisor/get-leave-details");
    return res.data;
  };
  ...
  const updateLeaveStatus = async ({ id, status }) => {
    const res = await api.post(`/supervisor/change-employee-status/${id}`, { status });
    return res.data;
  };
  ...
  const handleViewFile = async (fileId) => {
    try {
      ...
      const res = await api.get(`/supervisor/get-file-pdf/${fileId}`);
  ...
  // 👇 YAHAN CONFIRMATION POPUP KA HANDLER ADD KIYA HAI 👇
  const handleConfirmAction = () => {
    if (confirmModal.fileId && confirmModal.newStatus) {
      statusMutation.mutate({
        id: confirmModal.fileId,
        status: confirmModal.newStatus,
      });
    }
    // Action lene ke baad popup close kar do
    setConfirmModal({ isOpen: false, fileId: null, newStatus: null, statusLabel: "" });
  };
    ...
    <div className="bg-gray-50 min-h-screen ">
      ...
      <div className="flex flex-col md:flex-row justify-between items-center mb-6 gap-4">
        <div className="w-full flex items-center gap-3">
          <div>
            <h1 className="text-xl font-bold text-gray-900">Leave file approved</h1>
            <p className="text-sm text-gray-600">छुट्टी की फ़ाइल मंज़ूर</p>
          </div>
        </div>
              ...
              {isLoading ? (
                ...
                paginatedFiles.map((row, index) => {
                  ...
                  <tr key={row.id} className="hover:bg-gray-50 transition-colors">
                    ...
                    {/* --- YAHAN ONCHANGE MEIN BADLAAV KIYA HAI --- */}
                      <td className="px-6 py-4">
                        <select
                          value={
                            row.status == "2"
                          ...
                          onChange={(e) => {
                            ...
                            // Direct API call ki jagah pehle Popup open hoga
                            setConfirmModal({
                              isOpen: true,
                              fileId: row.id,
                              newStatus: statusMap[value],
                              statusLabel: value // 'Accepted' ya 'Rejected'
                            });
                          }}
      ...
      {/* --- CONFIRMATION MODAL (NAYA ADD KIYA HAI) --- */}
      {confirmModal.isOpen && (
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-[60] p-4 backdrop-blur-sm">
          <div className="bg-white rounded-lg p-6 max-w-sm w-full shadow-2xl animate-fadeIn">
            <h3 className="text-xl font-semibold text-gray-900 mb-2">Are you sure?</h3>
            <p className="text-gray-600 mb-6">
              Do you really want to mark this file as <strong className={confirmModal.statusLabel === 'Accepted' ? 'text-green-600' : 'text-red-600'}>{confirmModal.statusLabel}</strong>?
            </p>
            <div className="flex justify-end gap-3">
              <button
                onClick={() => setConfirmModal({ isOpen: false, fileId: null, newStatus: null, statusLabel: "" })}
                className="px-4 py-2 bg-gray-100 border border-gray-300 text-gray-700 rounded-lg hover:bg-gray-200 transition font-medium"
              >
                Cancel
              </button>
              <button
                ...
                className={`px-4 py-2 text-white rounded-lg font-medium transition shadow-sm ${
                  confirmModal.statusLabel === 'Accepted' 
                  ? 'bg-green-600 hover:bg-green-700' 
                  : 'bg-red-600 hover:bg-red-700'
                }`}
              >
                Yes, {confirmModal.statusLabel}
              </button>
            </div>
          </div>
        </div>
