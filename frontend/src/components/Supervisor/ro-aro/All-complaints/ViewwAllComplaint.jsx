...
const getDisplayLabel = (option) => {
  ...
  // ✅ PS
  if (role === "ps" && !option.subrole && !option.subrole_name) {
    roleLabel = "Private Secretary";
  }

  // ✅ APS
  else if (role === "aps" && !option.subrole && !option.subrole_name) {
    roleLabel = "Additional Private Secretary";
  }

  // ✅ Supervisor → subrole
  else if (role === "supervisor" && option.subrole?.label) {
    roleLabel = option.subrole.label;
  }

  // ✅ fallback subrole_name
  else if (option.subrole_name) {
    roleLabel = option.subrole_name;
  }

  // ✅ subrole object
  else if (option.subrole?.label) {
    roleLabel = option.subrole.label;
  }

  // ✅ normal role
  else if (option.role?.label) {
    roleLabel = option.role.label;
  }
...
//     setTakeFileConfirmConfig({ open: true, type: "assign" });
  ...
  const forwardComplaintMutation = useMutation({
     ...
     onError: (error) => {
        ...
        if (resData?.errors) {
          setFieldErrors(resData.errors); // 🔥 yahi pe OTP error ayega
      ...
      {complaintData.dob && (
        <div>
          ...
          <p className="text-gray-800 text-sm">
            {capitalizeFirstLetter(complaintData.cause_date) || "उपलब्ध नहीं"}
          </p>
        </div>
      )}
...
{/* 
                  ...
                  >
                    {forwardComplaintMutation.isPending
                      ...
                      : "Send / Mark"}
                        ...
                        <div className="mt-6 flex flex-col gap-3 bg-blue-50/50 border border-blue-100 p-4 rounded-xl">
  ...
  <div className="flex flex-col gap-3 text-sm text-slate-700 mt-1 pl-1">
    ...
    <p>
      ...
      Assign the file to yourself for further action.
    </p>
              ...
              <div className="space-y-4">
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                  ...
                  <div className="p-3 bg-gray-50 rounded border border-gray-100">
                    ...
                    <p className="text-gray-800 font-medium">
                      {complaintData.complainants?.[0]?.father_name || "miyC/k ugha"}
                    </p>
                  </div>
                  ...
                  <div className="p-3 bg-gray-50 rounded border border-gray-100">
                    ...
                    <p className="text-gray-800 font-medium">
                      {complaintData.complainants?.[0]?.occupation || "miyC/k ugha"}
                    </p>
                  </div>
                </div>
              </div>
                      ...
                      <div className="grid grid-cols-1 sm:grid-cols-2 gap-x-4 gap-y-4">
                        <div>
                          ...
                          <p className="text-sm text-gray-800">
                            {resp?.respondent_name || "miyC/k ugha"}
                          </p>
                        </div>
                        <div>
                          <p className="text-xs text-gray-500 font-semibold">
                        ...
                        <div className="sm:col-span-2">
                          ...
                          <p className="text-sm text-gray-800">
                            {resp?.current_address || "miyC/k ugha"}
                          </p>
                        </div>
                      </div>
                    </div>
