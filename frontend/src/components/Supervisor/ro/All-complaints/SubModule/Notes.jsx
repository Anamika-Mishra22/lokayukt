...
const UserID = Cookies.get("id"); // <-- Added for Edit Condition
...
const Notes = ({ complaint }) => {
  ...
  const allNotesRef = useRef(null); // <-- All notes ke liye
  const [isGeneratingAll, setIsGeneratingAll] = useState(false); // <-- Loader ke liye
  const singleNoteRef = useRef(null); // <-- Single note print/download ke liye
  const [printNoteData, setPrintNoteData] = useState(null); // <-- Jo note print hoga uska data
  const [generatingId, setGeneratingId] = useState(null); // <-- Single loader ke liye ID
  ...
  const [editNoteId, setEditNoteId] = useState(null); // <-- Added Edit State
 ...
 const getSafeSplitY = (ctx, width, startY, pageHeightPx, totalHeight) => {
    ...
    for (let y = scanRange - 1; y >= 0; y--) {
      let isRowWhite = true;
      // x = 30 se start kiya taaki left/right borders ignore ho jayein
 ...
 // --- 2. MASTER PDF GENERATOR (Universal) ---
const generatePdfDocument = async (ref = popupRef, elementId = "pdf-content-div") => {
    // 1. Ab hum 'ref' parameter ka use karenge, na ki sirf 'popupRef' ka
    const activeRef = ref && ref.current ? ref : popupRef; 
    
    if (!activeRef.current) return null;

    try {
      const elementsToHide = activeRef.current.querySelectorAll(".pdf-hide-section");
      ...
      const canvas = await html2canvas(activeRef.current, {
        ...
        useCORS: true,
        logging: false,
        windowHeight: activeRef.current.scrollHeight + 50,
        ...
        onclone: (document) => {
          ...
          if (el) {
            ...
            styles.innerHTML = `
            .pdf-page-container { min-height: 860px !important; padding-bottom: 20px !important; padding-top: 10px !important; }
            .pdf-bottom-signature { position: static !important; margin-top: auto !important; margin-bottom: 10px !important; }
            .draft-preview-content * { font-family: 'KrutiDev' !important; }
      ...
      while (currentY < imgHeight) {
        ...
        tempCanvas.getContext("2d").drawImage(canvas, 0, currentY, imgWidth, sliceHeight, 0, 0, imgWidth, sliceHeight);
        pdf.addImage(tempCanvas.toDataURL("image/png"), "PNG", 0, 10, pdfWidth, (sliceHeight * pdfWidth) / imgWidth);
        currentY = splitY;
        pageCount++;
      }
      return pdf;
...
const handleDownloadPdf = async () => {
    const pdf = await generatePdfDocument(popupRef, "pdf-content-div");
    if (pdf) {
...
// --- NAYE FUNCTIONS ALL NOTES KE LIYE ---
  const handleDownloadAllPdf = async () => {
    if (notesList.length === 0)
      return toast.error("No notes available to download.");

    setIsGeneratingAll(true);
    const toastId = toast.loading("Generating Complete PDF...");

    try {
      const pdf = new jsPDF("p", "mm", "a4");
      const notes = allNotesRef.current.querySelectorAll(".note-item");
      
      const pdfWidth = 210;
      const printableHeight = 297 - 20; // Top aur Bottom ki 10-10 margin hata kar

      for (let i = 0; i < notes.length; i++) {
        const canvas = await html2canvas(notes[i], {
          scale: 2,
          backgroundColor: "#ffffff",
          useCORS: true,
        });
        ...
        // Har naya note (pehle wale ko chhod kar) ek naye page se start hoga
        if (i > 0) pdf.addPage();

        let currentY = 0;

        // Agar note 1 page se bada hai, to ye loop usko split karke agle pages par daalega
        while (currentY < imgHeight) {
          const splitY = getSafeSplitY(ctx, imgWidth, currentY, printableHeightPx, imgHeight);
          const sliceHeight = splitY - currentY;
          ...
          pdf.addImage(tempCanvas.toDataURL("image/png"), "PNG", 0, 10, pdfWidth, pdfSliceHeight);
          
          currentY = splitY;
          
          // Agar abhi bhi is note ka content bacha hai, to uske liye aur naya page lo
      ...
      pdf.save(`All_Notings_${complaint?.file_number || "File"}.pdf`);
      toast.success("All Notes PDF Downloaded successfully!", {
        id: toastId,
      });
    } catch (err) {
  ...
  const handlePrintAll = async () => {
    if (notesList.length === 0)
      return toast.error("No notes available to print.");
    ...
    try {
      ...
      for (let i = 0; i < notes.length; i++) {
        ...
        // Naya note naye page se shuru
        if (i > 0) pdf.addPage();

        let currentY = 0;

        // Smart Splitter taaki lamba note properly break ho
        while (currentY < imgHeight) {
          const splitY = getSafeSplitY(ctx, imgWidth, currentY, printableHeightPx, imgHeight);
          const sliceHeight = splitY - currentY;
          ...
          // Remaining text of same note goes to a new page
          if (currentY < imgHeight) {
            pdf.addPage();
          }
        }
  ...
  // --- NAYE FUNCTIONS SINGLE NOTE KE LIYE ---
  const handleDownloadSingleNote = (item) => {
    setGeneratingId(item.id);
    setPrintNoteData(item);
    
    // Thoda time dete hain taaki hidden container mein data aa jaye (300ms)
    setTimeout(async () => {
      const pdf = await generatePdfDocument(singleNoteRef, "single-pdf-content-div");
      if (pdf) {
        pdf.save(`Noting_${complaint?.file_number || "File"}_Note_${item.id}.pdf`);
  ...
  const handlePrintSingleNote = (item) => {
    setGeneratingId(item.id);
    setPrintNoteData(item);
...
setTimeout(() => {
  ...
  const handleEditClick = async (noteId) => {
    try {
      setLoading(true);
      const res = await api.get(`/supervisor/note/${noteId}`);

      if (res.data.status) {
        const noteData = res.data.data;
        const blocksFromHtml = htmlToDraft(noteData.description || "");
        const { contentBlocks, entityMap } = blocksFromHtml;
        const contentState = ContentState.createFromBlockArray(contentBlocks, entityMap);

        setEditorState(EditorState.createWithContent(contentState));
        setNote(noteData.description || "");
        setEditNoteId(noteData.id);

        const docObj = documents.find((d) => d.id === noteData.d_id);
        setSelectedDoc(docObj ? docObj.file : "");

        setPageRanges([{
          from: noteData.range_from || "",
          to: noteData.range_two || ""
        }]);
  ...
  "pdf-content-div"
    ...
    try {
      let res;
      if (editNoteId) {
        res = await api.put(`/supervisor/notes/${editNoteId}`, payload);
      } else {
        res = await api.post("/supervisor/add-notes", payload);
      }

      if (res.data.status) {
        toast.success(editNoteId ? "Note Updated Successfully!" : "Note Added Successfully!");
          ...
          {notesList.length > 0 && (
            <>
              <button 
                ...
                className="bg-gray-100 border border-gray-300 text-gray-700 px-3 py-2 text-xs rounded-lg hover:bg-gray-200 transition flex items-center gap-2"
              >
                {isGeneratingAll ? <FaSpinner className="animate-spin" /> : <FaPrint />} Print All
              </button>
              <button 
                onClick={handleDownloadAllPdf} 
                disabled={isGeneratingAll}
                className="bg-blue-50 border border-blue-200 text-blue-700 px-3 py-2 text-xs rounded-lg hover:bg-blue-100 transition flex items-center gap-2"
              >
                {isGeneratingAll ? <FaSpinner className="animate-spin" /> : <FaDownload />} Download All
              </button>
            </>
          )}

          {complaint?.assign_to_ro && (
            <button
            ...
            notesList.map((item, index) => {
              ...
              return (
                <div key={item.id || index} className="border rounded-lg p-3 md:p-4 bg-gray-50">
                 {/* --- NAYA CLEAN HEADER DESIGN WITH PRINT/DOWNLOAD --- */}
               <div className="flex flex-col mb-1 w-full">
                    {/* Naam aur Post */}
                    <p className="text-gray-800 font-medium"> 
   {item?.forwarded_by_name}
{" "}
{item?.forward_by_user?.sub_role?.label
  ? `(${item.forward_by_user.sub_role.label})`
  : item?.forward_by_user?.role?.label
  ? `(${item.forward_by_user.role.label})`
  : ""}
                    </p>
                    
                    {/* Date aur Buttons ka container - w-full aur justify-between se sab set ho jayega */}
                    <div className="flex items-center justify-between w-full mt-1 flex-wrap">
                      ...
                      <div className="flex items-center gap-1">
                        <button 
                          onClick={() => handlePrintSingleNote(item)}
                          disabled={generatingId === item.id}
                          className="text-gray-500 hover:text-gray-800 p-1 hover:bg-gray-200 rounded transition disabled:opacity-50"
                          title="Print Note"
                        >
                          {generatingId === item.id ? <FaSpinner className="animate-spin w-3 h-3" /> : <FaPrint className="w-3 h-3" />}
                        </button>
                        <button 
                          onClick={() => handleDownloadSingleNote(item)}
                          disabled={generatingId === item.id}
                          className="text-blue-500 hover:text-blue-800 p-1 hover:bg-blue-100 rounded transition disabled:opacity-50"
                          title="Download Note"
                        >
                          {generatingId === item.id ? <FaSpinner className="animate-spin w-3 h-3" /> : <FaDownload className="w-3 h-3" />}
                        </button>

                        {/* --- EDIT BUTTON --- */}
                        {item?.added_by == UserID && new Date(item?.created_at) > new Date(complaint?.actions?.[0]?.created_at) ? (
                          <button 
                            onClick={() => handleEditClick(item.id)} 
                            className="text-blue-600 hover:text-blue-800 text-[11px] font-medium border border-blue-200 px-2 py-0.5 ml-1 rounded bg-blue-50 transition-colors"
                          >
                            Edit
                          </button>
                        ) : null}
        ...
        <div className="fixed inset-0 bg-black bg-opacity-40 flex items-center justify-center z-50 p-4 md:p-0">
          <div className="bg-white w-full md:w-11/12 h-full md:h-5/6 shadow-xl relative flex flex-col md:flex-row rounded-md overflow-hidden">
            ...
            <div className="flex-1 p-4 md:p-6 overflow-y-auto">
              <h2 className="text-lg font-semibold mb-4 md:mb-6">{editNoteId ? "Update Note / Noting" : "Add Note / Noting"}</h2>
              ...
              <div className="flex flex-col md:flex-row gap-4 mt-6">
                <div className="flex-1">
                  ...
                  >
                    ...
                    {documents.map((doc) => (
                      <option key={doc.id} value={doc.file}>{doc.title || "उपलब्ध नहीं"}</option>
                    ))}
                  </select>
...
`}</style>
              ...
              <div className="px-6 py-6 md:px-8 md:py-8 text-sm leading-relaxed text-gray-800 flex flex-col min-h-[400px] relative pdf-page-container">
                
  {/* --- CONTENT AREA --- */}
  <div
    className="flex-grow rounded-md kruti-input bg-white px-2 py-2 md:px-5 md:py-4 draft-preview-content"
  ...
  {/* --- FOOTER / SIGNATURE --- */}
  <div className="flex pt-4 mt-auto pdf-bottom-signature w-full">
    <div className="text-start text-xs text-gray-600">
      <p>{name} (RO) {new Date().toLocaleDateString("en-IN")}</p>
    </div>
  </div>
</div>
     ...
     {/* HIDDEN CONTAINER FOR 'ALL NOTES' PDF GENERATION */}
   {/* HIDDEN CONTAINER FOR 'ALL NOTES' PDF GENERATION */}
      <div className="absolute left-[-9999px] top-[-9999px] w-[800px]">
        <div ref={allNotesRef} id="all-pdf-content-div" className="bg-white">
          <div className="pdf-page-container">
            
            {notesList.map((item, index) => (
              <div 
                key={item.id || index} 
                // 👇 YAHAN MAGIC HAI: 1050px ek A4 page ki exact height hoti hai 800px width ke hisaab se
                style={{ minHeight: '1050px', display: 'flex', flexDirection: 'column' }} 
                className="p-8 note-item mb-4 bg-white border-b-2 border-transparent"
              >
                
                {/* 1. PAGE HEADER (Har page par File No. dikhega) */}
                {/* <h2 className="text-xl font-bold mb-6 text-center border-b pb-4">
                  File No: {complaint?.file_number || "N/A"} - Noting #{notesList.length - index}
                </h2> */}
                
                {/* 2. NOTE INFO */}
                <div className="flex justify-between items-center mb-6">
                  {/* <p className="font-semibold text-gray-800 text-sm underline">
                    Note by: {item?.forwarded_by_name} {item?.forward_by_user?.sub_role?.label ? `(${item.forward_by_user.sub_role.label})` : ''}
                  </p> */}
                ...
                {/* 3. MAIN CONTENT (flex-grow bachi hui saari jagah le lega) */}
                <div
                  className="kruti-input draft-preview-content flex-grow"
                  dangerouslySetInnerHTML={{
                ...
                {/* 4. FOOTER / SIGNATURE (mt-auto isko dhakel kar page ke sabse niche set kar dega) */}
                {/* 4. FOOTER / SIGNATURE (mt-auto isko dhakel kar page ke sabse niche set kar dega) */}
<div className="flex pt-4 mt-auto pdf-bottom-signature w-full">
  <div className="text-start text-xs text-gray-600">
    <p>
      {item?.forwarded_by_name} {item?.forward_by_user?.sub_role?.label ? `(${item.forward_by_user.sub_role.label})` : ''} {new Date(item.created_at).toLocaleDateString("en-IN")}
    </p>
  </div>
</div>
     ...
     {/* HIDDEN CONTAINER FOR 'SINGLE NOTE' PDF GENERATION */}
      <div className="absolute left-[-9999px] top-[-9999px] w-[800px]">
        <div ref={singleNoteRef} id="single-pdf-content-div" className="bg-white">
          {printNoteData && (
            // 👇 Yahan style mein 'display: flex', 'flexDirection: column', aur 'height: 1050px' fixed hai
           <div 
              className="pdf-page-container p-8" 
               style={{
    height: '1050px',
    display: 'flex',
    flexDirection: 'column'
            ...
            >
              ...
              {/* Content area: flex-grow yahan content ko push karega */}
              <div
                className="kruti-input draft-preview-content flex-grow"
                dangerouslySetInnerHTML={{
                  __html: printNoteData.description
                    .replace(/<li>/g, '<li style="font-family: Arial, sans-serif !important; margin-bottom: 0px !important; line-height: 1.0 !important;"><span style="font-family: \'KrutiDev\' !important; font-size: 22px;">')
                    .replace(/<\/li>/g, '</span></li>')
                    .replace(/<p>/g, '<p style="font-family: \'KrutiDev\' !important; font-size: 22px; margin-top: 0px !important; margin-bottom: 2px !important; line-height: 1.0 !important;">')
                }}
              ...
              {/* Footer: mt-auto isko bilkul niche le jayega */}
              <div className="flex pt-4 mt-auto pdf-bottom-signature w-full">
                <div className="text-start text-xs text-gray-600">
                  <p>
                    {printNoteData?.forwarded_by_name}
{" "}
{printNoteData?.forward_by_user?.sub_role?.label
  ? `(${printNoteData.forward_by_user.sub_role.label})`
  : printNoteData?.forward_by_user?.role?.label
  ? `(${printNoteData.forward_by_user.role.label})`
  : ""}
{" "}
{new Date(printNoteData.created_at).toLocaleDateString("en-IN")}
                  </p>
                </div>
              </div>
