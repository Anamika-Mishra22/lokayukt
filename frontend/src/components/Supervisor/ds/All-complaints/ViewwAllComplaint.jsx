...
const getRoleLabel = (option) => {
  ...
  if (role === "ps" && !option.subrole && !option.subrole_name) {
    return "Private Secretary";
  }

  // ✅ APS
  if (role === "aps" && !option.subrole && !option.subrole_name) {
    return "Additional Private Secretary";
  }

  // ✅ Supervisor → subrole
  if (role === "supervisor" && option.subrole?.label) {
    return option.subrole.label;
  }

  // ✅ fallback subrole_name
  if (option.subrole_name) {
    return option.subrole_name;
  }

  // ✅ subrole object
  if (option.subrole?.label) {
    return option.subrole.label;
  }

  // ✅ normal role
  if (option.role?.label) {
...
const ViewAllComplaint = () => {
  ...
  const pullBackMutation = useMutation({
    mutationFn: async ({ complaintId }) => {
      return api.post(`/supervisor/pull-back-by-ds/${complaintId}`);
    },
    onSuccess: (res) => {
  ...
  const assignToSelfMutation = useMutation({
    mutationFn: async ({ complaintId }) => {
      const res = await api.post(`/supervisor/assign-by-ds/${complaintId}`);
      return res.data;
    },
    onSuccess: (data) => {
  ...
  if (resData?.errors) {
    setFieldErrors(resData.errors); // ✅ OTP error show होगा
  ...
  />
              ...
              {activeTab === "draft" && (
                <DraftLetter complaint={complaintData} />
              )}
            </div>
...
{/* 
                  <button
                    ...
                    className="px-4 py-2 border border-gray-300 text-gray-700 rounded hover:bg-gray-50 text-sm ml-2 disabled:opacity-50 disabled:cursor-not-allowed"
                  >
...
>
  {dispose.isPending ? "Processing..." : "Dispose"}
                  ...
                  {complaintData.approved_rejected_by_ds == "1" ? (
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40">
          <div className="bg-white w-[420px] rounded-lg shadow-lg relative">
            ...
            <div className="flex justify-end gap-3 px-6 pb-2 ">
              ...
              <button
          onClick={() => assignToSelfMutation.mutate({ complaintId: id })}
        ...
        {fieldErrors.otp[0]}
            ...
            <div className="flex justify-end gap-3">
              ...
              >
                {markAsReceivedMutation.isPending ||
                forwardComplaintMutation.isPending ||
                assignToSelfMutation.isPending || // ✅ Check added
                pullBackMutation.isPending // ✅ Check added
                  ? "Processing..."
                  : confirmConfig.type === "assign" ||
                    confirmConfig.type === "pullback"
                  ? "Yes"
