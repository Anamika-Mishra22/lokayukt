...
const ViewRtiFile = () => {
  ...
  const { data: response, isLoading } = useQuery({
    ...
    queryFn: async () => {
      const res = await api.get(`/supervisor/get-leave-rti-details/${id}`);
      return res.data;
    },
    enabled: !!id
  });
  ...
  const sendPermissionMutation = useMutation({
    mutationFn: async ({ file_id, to_user_id }) => {
      const res = await api.post("/supervisor/rti-file-send", {
        file_id,
        to_user_id,
      });
      return res.data;
    ...
    onSuccess: (data) => {
      ...
      queryClient.invalidateQueries({ queryKey: ["rti-file-list", id] });
       setTimeout(() => {
         navigate("/pio/all-rti-file", { replace: true });
        }, 2000);
    },
    onError: (err) => {
    ...
    {isDropdownOpen && (
      <div className="absolute z-50 w-full mt-1 bg-white border rounded-md shadow-lg">
        ...
        <ul className="max-h-48 overflow-y-auto">
          {roleSubrole
            ?.filter((item) => {
              // Yahan aapke purane format ke hisab se search match karega
            ...
            .map((item) => (
              <li
                ...
                onClick={() => {
                  ...
                  setIsDropdownOpen(false); // Select karne par dropdown band
