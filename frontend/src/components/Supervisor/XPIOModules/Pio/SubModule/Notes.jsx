...
const Notes = ({ complaint }) => {
  ...
  const fetchNotes = async () => {
    ...
    try {
      setLoading(true);
      const res = await api.get(`/supervisor/get-personal-notes/${complaint.id}`);
      if (res.data.status) {
  ...
  const handleFinalSubmit = async () => {
    ...
    try {
      ...
      if (editNoteId) {
        ...
        res = await api.post("/supervisor/add-notes-employee", payload);
      }
  ...
  {/* RIGHT SIDE */}
  <button
    className="bg-blue-600 text-white px-4 py-2 text-sm rounded-lg hover:bg-blue-700 transition shadow-sm"
    onClick={() => {
      setEditNoteId(null); // --- RESET ADDED HERE ---
      setNote(""); 
      setEditorState(EditorState.createEmpty()); 
      setSelectedDoc("");
      setPageRanges([{ from: "", to: "" }]);
                ...
                <div key={item.id || index} className="border rounded-lg p-3 md:p-4 bg-gray-50">
                  <div className="flex flex-col md:flex-row justify-between md:items-start gap-2 md:gap-0">
                    ...
                    <div className="flex items-center gap-3">
                      ...
                      {/* --- EDIT BUTTON CONDITION ADDED HERE --- */}
...
`}</style>
              ...
              {/* --- SIGNATURE FIX: flex flex-col & min-h-[850px] ADDED HERE --- */}
