...
const Notes = ({ complaint }) => {
  ...
  // ✅ 1. FETCH DOCUMENTS VIA NEW API
  useEffect(() => {
    const fetchDocs = async () => {
      try {
        const res = await api.get(`/supervisor/get-letter-doc/${id}`);
  ...
  const fetchNotes = async () => {
    try {
      setLoading(true);
      const res = await api.get(`/supervisor/get-letter-notes/${id}`);
      if (res.data.status) {
  ...
  // ✅ 2. UPDATED DOC SELECTION (Directly using URL from New API State)
  const handleSelectDoc = (docId) => {
    ...
    if (doc && doc.url) {
      setPdfViewUrl(doc.url);
    } else {
      toast.error("PDF path not found");
    }
  };

  const handleViewDocFromNote = (docId, noteId) => {
    setActiveNoteId(noteId);   
    if (!docId) return;

    // पुराने 'get-file-preview' की जगह अब सीधे 'documents' स्टेट से url निकाली जा रही है 
    const docFromList = documents?.find((d) => d?.id?.toString() === docId.toString() || d?.file_name === docId);
    
    if (docFromList && docFromList.url) {
      setViewDocUrl(docFromList.url);
    } else {
  ...
  const handleFinalSubmit = async () => {
    ...
    try {
      const res = await api.post("/supervisor/add-letter-notes", payload);
  ...
  const getDocName = (dId) => {
    if (!documents?.length || !dId) return dId || null;
    const doc = documents.find((d) => d?.id?.toString() === dId.toString());
    return doc ? doc.file_name : dId; 
  };
              ...
              return (
                <div key={item?.id || index} className="border rounded-lg p-3 md:p-4 bg-gray-50">
                  <div className="flex flex-col md:flex-row justify-between md:items-start gap-2 md:gap-0">
                    <p className="text-gray-800 font-medium"> {item?.forwarded_by_name || "Unknown"}</p>
                      ...
                      <div className="mt-1 pl-2 border-l-2 border-blue-200 flex flex-col gap-1">
                        {referencedTitle && (
                          <div className="flex items-center gap-2 flex-wrap">
                            <button
                              onClick={() => handleViewDocFromNote(item?.d_id, item?.id)}
