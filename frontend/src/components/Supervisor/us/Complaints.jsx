  ...
  useEffect(() => {
    const fetchAllData = async () => {
      try {
        ...
        // Fetch subjects
        const subjectsResponse = await api.get(`/supervisor/subjects`);
        if (subjectsResponse.data.status === 'success') {
          setSubjects(subjectsResponse.data.data);
        }

        // Fetch complaint types
        const complaintTypesResponse = await api.get(`/supervisor/complainstype`);
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const response = await axios.post(
        `${BASE_URL}/supervisor/complaints`,
        submitFormData,
        {
          headers: {
