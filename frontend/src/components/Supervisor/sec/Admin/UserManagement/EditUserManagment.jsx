...
const [showConfirmPassword, setShowConfirmPassword] = useState(false);
  ...
  const isAssistantPrivateSecretary = String(formData.role_id) === "12";
  ...
  useEffect(() => {
    const fetchAllData = async () => {
      try {
        const userResponse = await api.get(`/supervisor/edit-users/${id}`);
        if (userResponse.data.status === true) {
          ...
          setFormData({
            ...
            password_confirmation: "",
             otp: userData.otp || "" ,
              is_pio: Number(userData.is_pio) || 0
          });
        } else {
  ...
  Number(formData.role_id) === 12 ||
  Number(formData.role_id) === 13 ||
  Number(formData.role_id) === 14
) {
         updatePayload.ps_parent = formData.ps_parent;
         updatePayload.sub_role_id = "";
      }
...
const isPageLoading =
  ...
  if (isPageLoading) {
  return (
    <div className="p-10 flex justify-center items-center h-screen  text-gray-500">
            Loading...
          </div>
  );
}
    ...
    {errors.ps_parent && (
      <p className="mt-1 text-sm text-red-600">
        {errors.ps_parent}
              ...
              {(isPersonalSecretary || isAssistantPrivateSecretary || isSupervisor ||  formData.role_id === "13" || formData.role_id === "14") && (
                <div>
                  <label
                    htmlFor="ps_parent"
                    className="block text-xs sm:text-sm font-medium text-gray-700 mb-1"
...
<div className="flex items-center h-full relative top-3">
  ...
  >
    ...
    <div className="relative flex items-center justify-center shrink-0">
      <input
        type="checkbox"
        checked={Number(formData.is_pio) === 1}
        onChange={(e) =>
          setFormData((prev) => ({
            ...prev,
