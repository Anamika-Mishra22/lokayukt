...
const isAssistantPrivateSecretary = formData.role_id === "12";
const needsPsParent = isPersonalSecretary || (isSupervisor && formData.sub_role_id);
const isRole13or14 = formData.role_id === "13" || formData.role_id === "14";
  ...
  useEffect(() => {
    const fetchAllData = async () => {
      try {
        ...
        // Fetch departments
        const departmentsResponse = await api.get(`/supervisor/department`);
        ...
        // Fetch designations
        const designationsResponse = await api.get(`/supervisor/designation`);
  ...
  useEffect(() => {
    async function fetchRoles() {
      try {
        const res = await api.get("/supervisor/get-roles");
  ...
  useEffect(() => {
    async function fetchSubRoles() {
      ...
      try {
        const res = await api.get(`/supervisor/get-sub-roles/${formData.role_id}`);
...
// ✅ Role के according fields add करें
  // APS ya PS select ho toh unhe sub_role_id empty rakhna hai aur ps_parent bhejna hai
  if (isPersonalSecretary || isAssistantPrivateSecretary || isRole13or14) {
    payload.ps_parent = formData.ps_parent || "";
    payload.sub_role_id = ""; 
  } else {
    payload.sub_role_id = formData.sub_role_id || "";
    
    // Agar supervisor wagera ko ps_parent chahiye toh unke liye
    if (needsPsParent) {
      payload.ps_parent = formData.ps_parent || "";
    } else {
      payload.ps_parent = "";
  ...
  try {
    const response = await api.post('/supervisor/add-user', payload); 
...
const fetchLokayukt = async () => {
  const res = await api.get('/supervisor/get-lokayukt-uplokayukt')
...
// };
  ...
  {!isPersonalSecretary && !isAssistantPrivateSecretary && (
    <div>
  ...
  {/* 👇 Condition: Sabke liye dikhega, bas Admin (id: 1), Supervisor aur APS pe HIDE ho jayega */}
{!(String(formData.role_id) === "1" || isSupervisor || isAssistantPrivateSecretary || isPersonalSecretary) && (
  <div>
    <label htmlFor="ps_parent" className="block text-xs sm:text-sm font-medium text-gray-700 mb-1">
       {/* 👇 Dynamic Label: PS ke liye alag, baaki sabke liye normal */}
       {isPersonalSecretary
...
{(isPersonalSecretary || isAssistantPrivateSecretary || (isSupervisor && selectedSubRoleLabel)) && (
  <div>
    <label htmlFor="ps_parent" className="block text-xs sm:text-sm font-medium text-gray-700 mb-1">
       {/* Conditional Label APS aur PS dono ke liye */}
       {isPersonalSecretary
        ? "PS Under Hon' Lokayukt/Uplokayukt *"
        : isAssistantPrivateSecretary
          ? "APS Under Hon' Lokayukt/Uplokayukt/Secretary *"
          : "Under Officer *"}
    </label>
...
>
  ...
  >
    {/* 'ftyk pqusa' = KrutiDev mein 'जिला चुने' */}
  ...
  >
    {/* 'foHkx pqusa' = KrutiDev mein 'विभाग चुने' */}
...
{/* Is PIO Card */}
<div className="flex items-center h-full relative top-3">
  <label
    className={`
      ...
      ${
        formData.is_pio === 1
          ? "border-[#123463] bg-gradient-to-r from-blue-50 to-indigo-50 shadow-md"
          : "border-gray-300 bg-white hover:border-[#123463]/50 hover:shadow-md"
      }
    `}
  >
    {/* Checkbox */}
    <div className="relative flex items-center justify-center shrink-0">
      <input
        type="checkbox"
        checked={formData.is_pio === 1}
        onChange={(e) =>
          setFormData((prev) => ({
            ...prev,
            is_pio: e.target.checked ? 1 : 0,
          }))
        }
        className="
          peer
          appearance-none
          w-5 h-5
          border-2 border-[#123463]
          rounded-md
          cursor-pointer
          checked:bg-[#123463]
          checked:border-[#123463]
    ...
    <div className="flex flex-col leading-tight">
      <span className="text-sm font-semibold text-gray-800 tracking-wide">
        (PIO) Check for Public Information Officer
      </span>
