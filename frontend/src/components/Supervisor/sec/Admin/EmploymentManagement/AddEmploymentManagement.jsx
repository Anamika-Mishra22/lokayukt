// pages/AddEmploymentManagement.js
...
const handleSubmit = async (e) => {
  ...
  // ✅ Role के according fields add करें
  if (isPersonalSecretary || isAssistantPrivateSecretary) {
    // Personal Secretary YA APS के लिए
    payload.ps_parent = formData.ps_parent || "";
    payload.sub_role_id = ""; // empty string
  } else {
    // दूसरे roles के लिए
    payload.sub_role_id = formData.sub_role_id || "";
    // Agar needsPsParent true hai toh supervisor ka parent bhi jana chahiye
    if (needsPsParent) {
      payload.ps_parent = formData.ps_parent || "";
    } else {
      payload.ps_parent = "";
  ...
  try {
    const response = await api.post('/supervisor/add-employee', payload); 
...
{/* Yahan bhi APS condition add karo 👇 */}
...
export default AddEmploymentManagement;
