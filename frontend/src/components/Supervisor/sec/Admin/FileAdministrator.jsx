...
//     queryKey: ['topics'],
//     queryFn: async () => {
//       const res = await api.get('/supervisor/topics');
...
//     queryKey: ['filetypes'],
//     queryFn: async () => {
//       const res = await api.get('/supervisor/filetypes');
//       return res.data.data || res.data;
//     }
//   });
...
//     queryKey: ['budgets'],
//     queryFn: async () => {
//       const res = await api.get('/supervisor/budgets');
//       return res.data.data || res.data;
//     }
//   });
...
//     mutationFn: (data) => api.post('/supervisor/add-topic', data),
...
//     mutationFn: ({ id, data }) => api.post(`/supervisor/edit-topic/${id}`, data),
...
//     mutationFn: (id) => api.post(`/supervisor/delete-topic/${id}`),
...
//     mutationFn: (data) => api.post('/supervisor/add-filetype', data),
...
//     mutationFn: ({ id, data }) => api.post(`/supervisor/edit-filetype/${id}`, data),
...
//     mutationFn: (id) => api.post(`/supervisor/delete-filetype/${id}`),
...
//     mutationFn: (data) => api.post('/supervisor/add-budget', data),
//     onSuccess: () => {
...
//     mutationFn: ({ id, data }) => api.post(`/supervisor/edit-budget/${id}`, data),
//     onSuccess: () => {
//       queryClient.invalidateQueries(['budgets']);
...
//     mutationFn: (id) => api.post(`/supervisor/delete-budget/${id}`),
//     onSuccess: () => {
//       queryClient.invalidateQueries(['budgets']);
...
const FileAdministrator = () => {
  ...
  const addFileTypeMutation = useMutation({
    ...
    onError: (err) => {
      if (err.response?.data?.errors) {
        ...
        toast.error(err.response?.data?.message || 'Error adding file type');
      }
    }
  });
  ...
  const editFileTypeMutation = useMutation({
    ...
    onError: (err) => {
      if (err.response?.data?.errors) {
        ...
        toast.error(err.response?.data?.message || 'Error updating file type');
      }
    }
  });
  ...
  const isSubmitting = addFileTypeMutation.isPending || editFileTypeMutation.isPending;

  const confirmDelete = async () => {
    try {
      setIsDeleting(true);
      await deleteFileTypeMutation.mutateAsync(editId);
      closeModal();
    } catch (err) {
      console.log(err);
    } finally {
          ...
          <div className="bg-white rounded-xl shadow-2xl w-full max-w-md">
            ...
            <div className="px-6 py-4 border-b flex justify-between items-center bg-gray-50 rounded-t-xl">
              <h3 className="font-bold text-gray-800">
                {modalType === 'delete' 
                  ? 'Delete File Type' 
                  : `${modalMode === 'add' ? 'Add' : 'Edit'} File Type`
                }
              </h3>
              <button onClick={closeModal}><FaTimes className="text-gray-400 hover:text-red-500" /></button>
            </div>
            ...
            {modalType === 'form' ? (
              <form onSubmit={handleSubmit} className="p-6 space-y-4">
                <div>
                  ...
                  <input
                    value={nameInput}
                    placeholder="फ़ाइल टाइप का नाम दर्ज करें"
                    onChange={(e) => setNameInput(e.target.value)}
                    className="w-full kruti-input px-3 py-2 border rounded-lg placeholder:text-sm placeholder:text-gray-400"
                  />
                  {formErrors.name && <p className="text-red-500 text-xs mt-1">{formErrors.name[0]}</p>}
                </div>
