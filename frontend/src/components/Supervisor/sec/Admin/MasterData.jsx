...
const AddModal = ({ isOpen, onClose, activeTab }) => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      switch (activeTab) {
        case 'districts':
          endpoint = `/supervisor/add-district`;
          requestData = { district_name: formData.name, dist_name_hi: formData.name, district_code: formData.code };
          break;
        case 'departments':
          endpoint = `/supervisor/add-department`;
          requestData = { name: formData.name, name_hindi: formData.name, dept_code: formData.code };
          break;
        case 'categories':
          endpoint = `/supervisor/add-category`;
          requestData = { name: formData.name, name_h: formData.name };
          break;
        case 'designations':
          endpoint = `/supervisor/add-designation`;
          requestData = { name: formData.name, name_h: formData.name };
          break;
        case 'complaint-types':
          endpoint = `/supervisor/add-complainstype`;
          requestData = { name: formData.name, name_h: formData.name, description: formData.description };
          break;
        case 'rejection-reasons':
          endpoint = `/supervisor/add-rejection`;
          requestData = { name: formData.name, name_h: formData.name, description: formData.description };
          break;
        default: return;
      }
...
const MasterData = () => {
  ...
  const handleBackupDownload = async () => {
    try {
      setDownlode(true);
      const response = await api.get("/supervisor/download-backup", { responseType: "blob" });
  ...
  const fetchMasterData = async (tab) => {
    let endpoint = '';
    switch (tab) {
      case 'districts': endpoint = '/supervisor/all-district'; break;
      case 'departments': endpoint = '/supervisor/department'; break;
      case 'categories': endpoint = '/supervisor/categories'; break;
      case 'designations': endpoint = '/supervisor/designation'; break;
      case 'complaint-types': endpoint = '/supervisor/complainstype'; break;
      case 'rejection-reasons': endpoint = '/supervisor/rejections'; break;
      default: return [];
    }
  ...
  const handleDeleteConfirm = async () => {
    ...
    try {
      let endpoint = '';
      switch (activeTab) {
        case 'districts': endpoint = `/supervisor/delete-district/${deletingItem.id}`; break;
        case 'departments': endpoint = `/supervisor/delete-department/${deletingItem.id}`; break;
        case 'categories': endpoint = `/supervisor/delete-category/${deletingItem.id}`; break;
        case 'designations': endpoint = `/supervisor/delete-designation/${deletingItem.id}`; break;
        case 'complaint-types': endpoint = `/supervisor/delete-complainstype/${deletingItem.id}`; break;
        case 'rejection-reasons': endpoint = `/supervisor/delete-rejection/${deletingItem.id}`; break;
        default: return;
      }
  ...
  const masterDataTabs = [
    ...
    { id: 'categories', label: 'Category', labelHi: 'श्रेणी', icon: FaFileAlt, iconColor: 'text-green-600' },
