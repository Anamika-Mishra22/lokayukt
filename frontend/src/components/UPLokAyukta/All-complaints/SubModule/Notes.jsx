...
setTimeout(() => {
  ...
  const handleEditClick = async (noteId) => {
    try {
      setLoading(true);
      const res = await api.get(`/uplokayukt/note/${noteId}`);
  ...
  "pdf-content-div"
    ...
    try {
      let res;
      if (editNoteId) {
        res = await api.put(`/uplokayukt/notes/${editNoteId}`, payload);
      } else {
        res = await api.post("/uplokayukt/add-notes", payload);
      }
...
`}</style>
  ...
  <div className="flex pt-4 mt-auto pdf-bottom-signature w-full">
    <div className="text-start text-xs text-gray-600">
      <p>{name} (Hon’ble UpLokayukt) {new Date().toLocaleDateString("en-IN")}</p>
    </div>
  </div>
</div>
