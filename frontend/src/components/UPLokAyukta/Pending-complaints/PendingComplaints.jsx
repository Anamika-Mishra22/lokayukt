...
const ForwardModal = ({ 
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const response = await api.post(`/uplokayukt/forward-by-ds-js/${complaintId}`, {
...
const PendingComplaints = () => {
  ...
  const fetchPendingComplaints = async () => {
    ...
    try {
      const response = await api.get("/uplokayukt/all-pending-complaints");
  ...
  const handleViewDetails = (e, complaintId) => {
    e.stopPropagation();
    navigate(`/uplokayukt/pending-complaints/view/${complaintId}`);
  ...
  const handleForwardSubmit = (forwardedComplaintId) => {
    // Refresh the current tab data after forwarding
    switch(activeTab) {
      case 'all':
        fetchAllComplaints();
        break;
  ...
  const canForward = (complaint) => {
    // If complaint is approved by uplokayukt, it's already forwarded
